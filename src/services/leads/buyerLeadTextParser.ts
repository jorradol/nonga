...
const NAME_STOP =
  /\s+(?:จัด(?:ไฟ)?แนนซ์|ผ่อน(?:ไฟ)?แนนซ์|ไฟแนนซ์|เงินสด|ซื้อสด|จ่ายสด|งบ(?:ประมาณ)?|เสนอ(?:ราคา)?|ราค(?:า)?(?:ที่)?เสนอ|ติดต่อ|โทร(?:ได้)?|สะดวก|ยังไม่แน่ใจ)/i;
...
const BUDGET_PRICE_PATTERN =
  /(?:งบ(?:ประมาณ)?|ไม่เกิน)\s*([\d,.]+(?:\s*แส(?:น)?(?:\s*(?:ห(?:้)?า|50))?|\s*ล้าน|\s*บาท)?)/gi;

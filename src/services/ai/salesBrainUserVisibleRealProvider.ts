...
/** v14.2 — Thai UX naturalness and sales-assistant structure tuning (owner-only quality line). */
export const USER_VISIBLE_THAI_UX_TUNING_SLICE_ID = "v14.2";
...
function buildScenarioAnswerGuidance(
  ...
  switch (scenario) {
    case "budget":
      return `สรุปตรงคำถามก่อน 1 ประโยค แล้วค่อยแนะนำรถจาก listing (${cardCount || "หลาย"} คัน) แบบไม่ซ้ำ พร้อมเหตุผลสั้น ๆ 1-2 ข้อต่อคัน`;
    case "finance":
    ...
    case "ev":
      return [
        "ตอบเรื่องรถไฟฟ้า/EV/แบต/ชาร์จแบบเข้าใจง่าย จากข้อมูล listing ถ้ามี",
    ...
    default:
      return "ตอบครบประเด็น โดยสรุปตรงคำถามก่อน แล้วเหตุผลสั้น ๆ 1-2 ข้อ ไม่ยาวเกินจำเป็น";
  }
}
...
function buildUserVisibleBuyerSystemInstruction(
  ...
  return [
    ...
    `โหมดคุณภาพภาษาไทย ${USER_VISIBLE_THAI_UX_TUNING_SLICE_ID}: ตอบให้ธรรมชาติ อ่านลื่น สุภาพ อบอุ่น แบบผู้ช่วยขายรถมือสองที่จริงใจและไม่ยัดเยียด.`,
    ...
    "โครงสร้างคำตอบที่ต้องการ: (1) สรุปตรงคำถามก่อน (2) ให้เหตุผลสั้น ๆ ที่โยงกับข้อมูลรถ/งบ/เงื่อนไขผู้ใช้ (3) เตือนความเสี่ยงหรือข้อควรเช็กแบบสั้น (4) ปิดท้ายด้วยทางเลือกที่ปลอดภัย.",
    "ความยาวเป้าหมาย 4-7 ประโยค กระชับแต่ไม่ห้วน หลีกเลี่ยงย่อหน้ายาวหรือรายการแข็งเกินไป.",
    scenarioGuidance,

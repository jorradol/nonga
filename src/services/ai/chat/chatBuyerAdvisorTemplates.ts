...
export const BUYER_ADVISOR_PATTERNS: { topic: BuyerAdvisorTopic; re: RegExp }[] = [
  ...
  {
    topic: "paymentSafety",
    // v7.5.1 — broadened so "โอนมัดจำก่อนดูรถไหม" / "โอนเงินมัดจำก่อน…" are caught
    // (the earlier pattern missed the โอน→มัดจำ→ก่อน→ดูรถ ordering).
    re: /โอน(?:เงิน)?(?:ค่า)?(?:มัดจำ)?.{0,4}ก่อน(?:ดู|เห็น|เจอ)รถ|(?:โอน|วาง|จ่าย)?(?:เงิน|ค่า)?มัดจำ(?:ก่อน(?:ดู(?:รถ)?)?)?(?:ดี|ปลอดภัย|ได้)?(?:ไหม|มั้ย)|จองรถ(?:ก่อน)?(?:ดี|ปลอดภัย)?(?:ไหม|มั้ย)|(?:กลัว|ระวัง)(?:จะ)?(?:โดน|ถูก)?(?:โกง|หลอก)|(?:โดน|ถูก)(?:โกง|หลอก)(?:รถ)?(?:ทำไง|ยังไง)?|มิจฉาชีพ(?:ขายรถ)?|(?:โอน|มัดจำ|จอง).{0,12}ปลอดภัย(?:ไหม|มั้ย)|ปลอดภัย(?:ไหม|มั้ย).{0,12}(?:โอน|มัดจำ|จอง)/i,
  },
  {
    topic: "hiddenCosts",

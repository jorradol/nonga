...
const SELF_PROTECTION_PATTERNS: Array<{ re: RegExp; reason: string }> = [
  ...
  {
    re: /(?:แสดง|เปิดเผย|ส่ง|reveal|show|dump|print).{0,40}(?:system\s*prompt|system\s*instruction|developer\s*instruction|คำสั่งระบบ|พรอมป์ระบบ)/i,
...
function buildSelfProtectionReply(hasAutomotiveAsk: boolean): string {
  if (hasAutomotiveAsk) {
    return [
      "ขอไม่เปิดเผยคำสั่งระบบ ข้อมูลภายใน หรือความลับใด ๆ นะคะ",
      "ถ้ามีคำถามเรื่องรถที่ปลอดภัย เล่าอาการหรือสิ่งที่อยากรู้มาได้เลย เดี๋ยวช่วยต่อให้",
    ].join("\n");
  }
  return [
    "ขอไม่เปิดเผยคำสั่งระบบ ความลับ หรือข้อมูลภายในของระบบนะคะ",
    "น้องเอช่วยเรื่องรถในขอบเขตที่ปลอดภัยได้ — ถามอาการ ดูแลรถ หรือเลือกซื้อรถมาได้เลย",
  ].join("\n");
}
...
function buildEmergencyGuidance(reason: string): string {
  return [
    ...
    "คำเตือนสั้น ชัด เป็นลำดับ ไม่ยาวเกินเหตุ และห้ามใช้ ปังปุริเย่!",
  ].join("\n");
}
...
function buildCautionGuidance(reason: string): string {
  return [
    ...
    "อย่าใส่คำเตือนยาวเกินเหตุ และห้ามใช้ ปังปุริเย่! ในบริบทเสี่ยง",
  ].join("\n");
}

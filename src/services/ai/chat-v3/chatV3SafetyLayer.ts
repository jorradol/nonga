...
function buildEmergencyGuidance(reason: string): string {
  ...
  if (reason === "brake_failure_in_motion") {
    lines.push(
      "เบรกจม/เบรกเสียขณะขับ: ตั้งสติ ถอนคันเร่ง ประคองทิศทาง · เปิดไฟฉุกเฉินเมื่อปลอดภัย · กดแป้นเบรกซ้ำอย่างมีจังหวะเพื่อประเมินแรงดัน (ห้ามใช้สูตร “ย้ำเบรกรัว ๆ” เป็นคำตายตัว) · ลดเกียร์ช่วยชะลอโดยไม่เปลี่ยนรุนแรง · ใช้เบรกจอดอย่างค่อยเป็นค่อยไป · มองหาที่ปลอดภัย · หลังหยุดห้ามขับต่อ",
      "ห้ามแนะนำให้ดับเครื่องเป็นขั้นตอนทั่วไปขณะรถยังเคลื่อนที่ เพราะอาจลดแรงช่วยพวงมาลัย/เบรก",
      "ห้ามรับรองว่าขั้นตอนเดียวใช้ได้กับรถและความขัดข้องทุกประเภท"
    );
  }
  return lines.join("\n");
}

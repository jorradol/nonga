/**
 * v6.5W — Gate D Staging Real Provider Pilot Smoke Pre-Execution Approval Packet (static validation only)
 * npm run test:v65w-gate-d-staging-real-provider-pilot-smoke-pre-execution-approval-packet
 */
...
const AUTO_EXEC_AUTH_PATTERNS = [
  ...
  /v6\.5W\s+captures\s+approval\s+for\s+Gate\s+D/i,
  /v6\.5W\s+authorizes\s+Gate\s+D\s+execution/i,
  /v6\.5W\s+authorizes\s+staging\s+pilot\s+smoke/i,
  /v6\.5W\s+approves\s+production\s+pilot/i,
  /execution\s+approved\s+for\s+production/i,
  /deploy\s+approved\s+for\s+production/i,
  /Gate\s+D\s+is\s+approved\s+by\s+v6\.5W\s+alone/i,
  /v6\.5W\s+approved\b/i,
];

const BAD_GATE_D_APPROVED_PATTERNS = [
  /Gate D Approved\?\s*=\s*Yes/i,
  /Gate D Executed\?\s*=\s*Yes/i,
  /Execution Allowed from v6\.5W\?\s*=\s*Yes/i,
  /Gemini Invocation in v6\.5W\?\s*=\s*Yes/i,
  /Prompt\/Provider Payload Sent in v6\.5W\?\s*=\s*Yes/i,
  ...
  /pilot smoke has been executed/i,
  /execution authorized by v6\.5W/i,
];

const HEAD_SHA = "c788c0173fb9cfec06fb2bec967fb5f706567b15";
...
const selfSrc = readFileSync(
  "scripts/test-v65w-gate-d-staging-real-provider-pilot-smoke-pre-execution-approval-packet.mts",
  "utf8"
);
...
{
  ok("v6.5W doc exists", doc.length > 8000);
  ok("doc v6.5W label", doc.includes("v6.5W"));
...
// --- safety positioning ---
{
  ok("doc HEAD c788c01", doc.includes(HEAD_SHA) || doc.includes("c788c01"));
  ok("doc staging bundle", doc.includes(STAGING_BUNDLE));
  ok("doc Real Gemini OFF", /Real Gemini.*OFF|OFF.*Real Gemini/i.test(doc));
  ok(
    "doc no Gate D execution in v6.5W",
    /no Gate D execution in v6\.5W|does not execute Gate D/i.test(doc)
  );
  ok(
    "doc no Gemini invocation in v6.5W",
    /no Gemini invocation|Gemini Invocation in v6\.5W\?\s*=\s*No|does not invoke Gemini/i.test(
      doc
    )
  );
  ok(
    "doc no prompt provider payload sending in v6.5W",
    /no prompt\/provider payload sending|Prompt\/Provider Payload Sent in v6\.5W\?\s*=\s*No|does not send prompts or provider payloads/i.test(
      doc
    )
  );
  ok(
  ...
  ok(
    "doc no env update",
    /no env update|Env Updated in v6\.5W\?\s*=\s*No/i.test(doc)
  );
  ok(
    "doc no Cloud Run env update",
    /no Cloud Run env update|Cloud Run Env Updated in v6\.5W\?\s*=\s*No/i.test(doc)
  );
  ok(
    "doc no gcloud execution",
    /no gcloud execution|gcloud Executed in v6\.5W\?\s*=\s*No|does not execute gcloud/i.test(doc)
  );
  ok(
  ...
  ok(
    ...
    /no Firestore write|Firestore write in v6\.5W/i.test(doc)
  );
  ok(
    "doc no AI log persistence",
    /no AI log persistence|AI log persistence in v6\.5W/i.test(doc)
  );
}
...
// --- status fields ---
{
  ...
  ok(
    "doc gemini invocation in v65w is no",
    /Gemini Invocation in v6\.5W\?\s*=\s*No|Gemini Invocation in v6\.5W\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
    "doc prompt provider payload sent in v65w is no",
    /Prompt\/Provider Payload Sent in v6\.5W\?\s*=\s*No|Prompt\/Provider Payload Sent in v6\.5W\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
  ...
  ok(
    "doc execution allowed from v65w is no",
    /Execution Allowed from v6\.5W\?\s*=\s*No|Execution Allowed from v6\.5W\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
  ...
  ok(
    "doc does not state Gate D executed",
    /Does not state Gate D executed|does not state Gate D executed/i.test(doc)
  );
}
...
// --- required sections ---
{
  ...
  ok("doc relationship to Gate C", /Relationship to Gate C/i.test(doc));
  ok("doc gate d definition", /Gate D Definition/i.test(doc));
  ok("doc gate d scope", /Gate D Scope/i.test(doc));
  ok("doc gate d non-scope", /Gate D Non-Scope/i.test(doc));
  ok("doc evidence carry-forward", /Evidence Carry-Forward/i.test(doc));
  ok(
    "doc adapter disabled-by-default confirmation",
    /Adapter Disabled-By-Default Confirmation/i.test(doc)
  );
  ok(
    "doc redaction guard carry-forward",
    /Redaction Guard Carry-Forward/i.test(doc)
  );
  ok("doc provider payload boundary", /Provider Payload Boundary/i.test(doc));
  ok("doc prompt input boundary", /Prompt\/Input Boundary/i.test(doc));
  ok(
    "doc response metadata-only boundary",
    /Response Metadata-Only Boundary/i.test(doc)
  );
  ok(
    "doc no raw provider response logging rule",
    /No Raw Provider Response Logging Rule/i.test(doc)
  );
  ok("doc no secret exposure rule", /No Secret Exposure Rule/i.test(doc));
  ok(
  ...
  ok("doc runtime switch expectation", /Runtime Switch Expectation/i.test(doc));
  ok("doc kill switch relationship", /Kill Switch Relationship/i.test(doc));
  ok("doc call cap retry policy", /Call Cap \/ Retry Policy/i.test(doc));
  ok("doc cost guard checklist", /Cost Guard Checklist/i.test(doc));
  ok(
    "doc privacy data handling checklist",
    /Privacy\/Data Handling Checklist/i.test(doc)
  );
  ok(
    "doc monitoring logging boundary",
    /Monitoring\/Logging Boundary/i.test(doc)
  );
  ok(
    "doc firestore persistence boundary",
    /Firestore \/ Persistence Boundary/i.test(doc)
  );
  ok("doc deploy exclusion", /Deploy Exclusion/i.test(doc));
  ok("doc pilot exclusion", /Pilot Exclusion/i.test(doc));
  ok("doc production exclusion", /Production Exclusion/i.test(doc));
  ok(
    "doc recommended conservative gate d slice",
    /Recommended Conservative Gate D Slice/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc conservative slice v65w exec label",
    /v6\.5W\.EXEC.*Staging Real Provider Pilot Smoke Minimal Candidate/i.test(doc)
  );
  ok(
    "doc required approval wording thai",
    /อนุมัติให้ execute Gate D: Staging Real Provider Pilot Smoke Minimal Candidate/i.test(
      doc
    )
  );
  ok(
    "doc metadata-only in approval wording",
    /metadata-only/i.test(doc)
  );
  ok(
    "doc v65w does not execute gate d",
    /v6\.5W does not execute Gate D/i.test(doc)
  );
  ok(
    "doc staging real provider pilot smoke",
...
// --- no secrets / PII / sensitive samples ---
{
  ...
  ok(
    "doc v65w does not authorize execution alone",
    /Execution Allowed from v6\.5W\?\s*=\s*No/i.test(doc)
  );
...
// --- package script ---
{
  ok(
    "package v65w script",
    pkg.includes(
      "test:v65w-gate-d-staging-real-provider-pilot-smoke-pre-execution-approval-packet"
    )
  );
  ok(
...
console.log(
  "\nDone v6.5W Gate D Staging Real Provider Pilot Smoke Pre-Execution Approval Packet tests."
);

/**
 * v6.5O — Staging Secret Wiring Execution Candidate Approval Packet (static validation only)
...
const AUTO_EXEC_AUTH_PATTERNS = [
  ...
  /v6\.5O\s+approves\s+production\s+pilot\s+automatically/i,
  /v6\.5O\s+authorizes\s+production\s+pilot\s+start/i,
  /v6\.5O\s+captures\s+approval\s+in\s+v6\.5O\s+slice/i,
  /v6\.5O\s+authorizes\s+secret\s+wiring\s+execution/i,
...
const BAD_APPROVED_PATTERNS = [
  ...
  /execution\s+authorized\s+by\s+v6\.5O/i,
  /secret\s+wiring\s+is\s+approved/i,
  /staging\s+secret\s+wiring\s+approved/i,
];

const HEAD_SHA = "f399ec25678dfde58d726a18fd0a761f7099418e";
...
const selfSrc = readFileSync(
  "scripts/test-v65o-staging-secret-wiring-execution-candidate-approval-packet.mts",
  "utf8"
);
...
{
  ok("v6.5O doc exists", doc.length > 15000);
  ok("doc v6.5O label", doc.includes("v6.5O"));
...
// --- safety positioning ---
{
  ok("doc HEAD f399ec2", doc.includes(HEAD_SHA) || doc.includes("f399ec2"));
  ...
  ok(
    "doc may execute from v65o is no",
    /May Execute from v6\.5O\?\s*=\s*No|May Execute from v6\.5O\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
...
// --- required sections ---
{
  ...
  ok(
    "doc what v65o is is not",
    /What v6\.5O Is \/ Is Not/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc Gate A selection carry-forward",
    /Gate A Selection Carry-Forward/i.test(doc)
  );
  ok("doc future gate scope", /Future Gate Scope/i.test(doc));
  ok("doc future gate non-scope", /Future Gate Non-Scope/i.test(doc));
  ok("doc secret wiring definition", /Secret Wiring Definition/i.test(doc));
  ok("doc staging-only boundary", /Staging-Only Boundary/i.test(doc));
  ...
  ok(
    "doc runtime user-visible change exclusion",
    /Runtime \/ User-Visible Change Exclusion/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc final Go/No-Go requirement",
    /Final Go\/No-Go Requirement/i.test(doc)
  );
  ok(
  ...
  ok("doc secret handling checklist", /Secret Handling Checklist/i.test(doc));
  ok(
  ...
  ok(
    "doc commands prohibited in v65o",
    /Commands Prohibited in v6\.5O/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc rollback cleanup boundary",
    /Rollback \/ Cleanup Boundary/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc recommended next version boundary",
...
// --- no secrets / PII / sensitive samples ---
{
  ...
  ok(
    "doc v65o does not authorize execution alone",
    /May Execute from v6\.5O\?\s*=\s*No|Execution Allowed\?\s*=\s*No/i.test(doc)
  );
...
// --- package script ---
{
  ok(
    "package v65o script",
    pkg.includes(
      "test:v65o-staging-secret-wiring-execution-candidate-approval-packet"
    )
  );
  ok(
...
console.log(
  "\nDone v6.5O Staging Secret Wiring Execution Candidate Approval Packet tests."
);

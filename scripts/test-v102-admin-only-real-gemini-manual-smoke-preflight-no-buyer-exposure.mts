 ...
 *
 * npm run test:v102:manual-smoke-preflight
 */
...
const NPM_SCRIPT_KEY = "test:v102:manual-smoke-preflight";
...
console.log("=== v10.2 Admin-Only Manual Smoke Preflight Validation ===\n");
...
ok("doc contains v10.2 label", /v10\.2/i.test(doc));
ok("doc contains preflight wording", /preflight/i.test(doc));
ok("doc contains no execution yet wording", /no execution yet/i.test(doc));

const REQUIRED_DOC_PHRASES: Array<[string, RegExp]> = [
  ["manual smoke path prep exists", /manual smoke path prep/i],
  ["operator runbook exists", /operator runbook/i],
  ...
  ["no real secret in repo", /ไม่มี secret\/API key จริงใน repo|no real API key\/secret in repo/i],
  ...
  ["no secret in repo policy", /ห้ามใส่ secret\/API key จริงใน repo|no real secret/i],
  ["no real customer pii", /ห้ามใช้ข้อมูลลูกค้าจริง|no real customer/i],
  ["no real phone vin plate pii", /เบอร์โทรจริง|VIN จริง|ทะเบียนจริง|PII จริง/i],
  ["kill switch rollback", /kill switch rollback|kill switch/i],
  ["quota cost cap low", /quota\/cost cap/i],
  ["log redaction", /log redaction/i],
  ["deterministic fallback", /deterministic fallback/i],
  ["manual smoke default off fail-closed", /default off|default OFF|fail-closed/i],
  ["placeholder env instruction", /<set-in-environment-only>/i],
  ["placeholder not real key warning", /ไม่ใช่ key จริง|ห้าม commit เป็น secret จริง/i],
  ["kill switch immediate toggle", /เปิด\/ปิดกลับทันที|ปิดได้ทันที/i],
  ["success criteria section", /success criteria/i],
  ["fail stop criteria section", /fail \/ stop criteria|fail \/ stop|หยุดทันที/i],
  ["rollback stop instruction", /rollback \/ stop instruction|rollback|stop instruction/i],
  ...
  ["conditional ready only", /CONDITIONAL READY/i],
...
ok("package includes v10.2 script key", pkg.includes(`"${NPM_SCRIPT_KEY}"`));
ok(
  "package points to v10.2 validator script",
  pkg.includes("scripts/test-v102-admin-only-real-gemini-manual-smoke-preflight-no-buyer-exposure.mts")
);
...
console.log(`\nDone v10.2 manual-smoke-preflight validation - ${pass} PASS, ${fail} FAIL.\n`);

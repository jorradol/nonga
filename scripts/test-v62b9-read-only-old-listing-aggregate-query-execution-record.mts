/**
 * v6.2B.9 — Read-only Old Listing Aggregate Query Execution Record (static validation only)
 * npm run test:v62b9-read-only-old-listing-aggregate-query-execution-record
 */
...
const V62B8_DOC =
  "docs/v6.2B.8-read-only-old-listing-cleanup-preflight.md";
const V62B7_DOC =
...
const HEAD_SHA = "fbe37179553a8236b24698aa39a8bdd628758379";
...
const selfSrc = readFileSync(
  "scripts/test-v62b9-read-only-old-listing-aggregate-query-execution-record.mts",
  "utf8"
);
const pkg = readFileSync("package.json", "utf8");
const v62b8Doc = readFileSync(V62B8_DOC, "utf8");
...
// --- doc exists + v6.2B.9 ---
{
  ...
  ok("doc v6.2B.9 label", doc.includes("v6.2B.9"));
  ok(
    "doc aggregate query execution",
    /aggregate query execution|Read-only Old Listing Aggregate/i.test(doc)
  );
  ok("doc HEAD fbe3717", doc.includes(HEAD_SHA) || doc.includes("fbe3717"));
  ok("doc references v62b8", /v6\.2B\.8/i.test(doc));
  ok("doc references v62b7", /v6\.2B\.7/i.test(doc));
  ok("doc read-only only", /read-only only|read-only aggregate/i.test(docLower));
}
...
// --- preflight metadata ---
{
  ...
  ok(
    ...
    /branch|head.*origin|fbe3717/i.test(docLower)
  );
  ok(
  ...
  ok(
    "metadata read-only GET query",
    /read-only.*GET|GET.*marketplace/i.test(docLower)
  );
  ok(
...
// --- redacted aggregate summary (executed counts, not PENDING) ---
{
  ok("aggregate summary section", /Aggregate Query Summary/i.test(doc));
  ok(
    "count total visible published 17",
    /totalVisiblePublished.*\*\*17\*\*|totalVisiblePublished: 17/i.test(doc)
  );
  ok(
  ...
  ok(
    "count unknown needs review 0",
    /unknownNeedsReview.*\*\*0\*\*|unknownNeedsReview: 0/i.test(doc)
  );
  ok(
    "count protected new real batch zero",
    /protectedNewRealBatch.*\*\*0\*\*|protectedNewRealBatch.*0/i.test(doc)
  );
  ok(
    "executed counts not pending",
    !/\|\s*totalVisiblePublished\s*\|\s*`PENDING`/i.test(doc)
  );
  ok(
    "redacted aggregate counts only",
    /redacted aggregate|counts only/i.test(docLower)
  );
  ok(
    "no per-car in git",
    /ห้ามบันทึกข้อมูลรายคัน|per-car.*forbidden/i.test(doc)
  );
}

// --- query criteria ---
{
  ok("query criteria section", /Query Criteria Applied/i.test(doc));
  ...
  ok("criteria ownerUid", /ownerUid|distinctOwnerCount/i.test(doc));
  ok("criteria status", /listingStatus|status/i.test(doc));
  ok("criteria test demo marker", /test\/demo marker/i.test(doc));
  ok(
    "criteria count only no expose",
    /count เท่านั้น|aggregate classification only/i.test(doc)
  );
}

// --- reference risk ---
{
  ok("reference risk section", /Reference Risk Summary/i.test(doc));
  ...
  ok(
    "risk payment settlement",
    /payment references|settlement references/i.test(docLower)
  );
  ok("reference risk needs-review", /referenceRisk.*needs-review|needs-review/i.test(doc));
...
// --- cleanup recommendation ---
{
  ...
  ok("rec FIX REQUIRED for this record", /This execution record.*FIX REQUIRED/i.test(doc));
...
// --- import gate ---
{
  ...
  ok(
    ...
    /cleanup.*approval|cleanup path/i.test(docLower)
  );
}

// --- read-only / no execution ---
{
  ok("read-only only", /read-only only|read-only aggregate/i.test(docLower));
  ok(
    "no runtime code",
    /docs\/tests\/package only|not in v6\.2B\.9/i.test(docLower)
  );
...
// --- forbidden scope ---
{
  ...
  ok(
    "no secrets access",
    /Secret access.*not done|secrets versions access/i.test(docLower)
  );
  ok(
  ...
  ok("compliance section", /Compliance.*v6\.2B\.9/i.test(doc));
}
...
// --- cross-ref v62b8 / v62b7 ---
{
  ok("v62b8 preflight exists", /v6\.2B\.8/i.test(v62b8Doc));
...
{
  ok(
    "package v62b9 script",
    pkg.includes(
      "test:v62b9-read-only-old-listing-aggregate-query-execution-record"
    )
  );
  ok(
...
console.log(
  "\nDone v6.2B.9 Read-only Old Listing Aggregate Query Execution Record tests."
);

/**
 * v19.3 owner future request type clarification packet validator
 * Static checks only. Clarification-only, future-request-type-clarification-only, review-only.
 *
 * npm run test:v19.3
 */
import { existsSync, readFileSync } from "node:fs";

const DOC_PATH = "docs/v19.3-owner-future-request-type-clarification-packet.md";
const FIXTURE_PATH = "docs/examples/v19.3-owner-future-request-type-clarification-packet.synthetic.json";
...
function isSafeNegativeOrProhibitionContext(line: string): boolean {
  ...
  const safeMarkers = [
    "clarification only",
    "future request type clarification only",
    "review only",
    "placeholder only",
    "not selected",
    "not approval",
...
const requiredSections = [
  ...
  "currentStatusAfterV192Section",
  "closedBaselineCarryForwardSection",
  "v193ScopeSection",
  "clarificationOnlyReminderSection",
  "futureRequestTypeClarificationOnlyReminderSection",
  ...
  "v19ClarificationPacketPurposeSection",
  "requestTypeClarificationSection",
  "requestTypeClarificationNotSelectionSection",
  "ownerDecisionNotIncludedSection",
  "noFuturePathSelectedSection",
  "noRequestTypeSelectedSection",
  "reviewOnlyRequestTypeClarificationSection",
  "executionRequestDraftClarificationSection",
  "realValueInsertionDraftClarificationSection",
  "ownerApprovalPacketClarificationSection",
  "controlledOneRunPrepClarificationSection",
...
const requiredPlaceholders = [
  ...
  "CURRENT_STATUS_AFTER_V192_PLACEHOLDER_ONLY",
  "CLOSED_BASELINE_CARRY_FORWARD_PLACEHOLDER_ONLY",
  "V193_SCOPE_PLACEHOLDER_ONLY",
  "CLARIFICATION_ONLY_REMINDER_PLACEHOLDER_ONLY",
  "FUTURE_REQUEST_TYPE_CLARIFICATION_ONLY_REMINDER_PLACEHOLDER_ONLY",
  ...
  "V19_CLARIFICATION_PACKET_PURPOSE_PLACEHOLDER_ONLY",
  "REQUEST_TYPE_CLARIFICATION_PLACEHOLDER_ONLY",
  "REQUEST_TYPE_CLARIFICATION_NOT_SELECTION_PLACEHOLDER_ONLY",
  ...
  "REVIEW_ONLY_REQUEST_TYPE_CLARIFICATION_PLACEHOLDER_ONLY",
  "EXECUTION_REQUEST_DRAFT_CLARIFICATION_PLACEHOLDER_ONLY",
  "REAL_VALUE_INSERTION_DRAFT_CLARIFICATION_PLACEHOLDER_ONLY",
  "OWNER_APPROVAL_PACKET_CLARIFICATION_PLACEHOLDER_ONLY",
  "CONTROLLED_ONE_RUN_PREP_CLARIFICATION_PLACEHOLDER_ONLY",
...
const clarificationCategories = [
  "FUTURE_REQUEST_TYPE_CLARIFICATION_REVIEW_ONLY_PLACEHOLDER",
  "FUTURE_REQUEST_TYPE_CLARIFICATION_EXECUTION_REQUEST_DRAFT_PLACEHOLDER",
  "FUTURE_REQUEST_TYPE_CLARIFICATION_REAL_VALUE_INSERTION_DRAFT_PLACEHOLDER",
  "FUTURE_REQUEST_TYPE_CLARIFICATION_OWNER_APPROVAL_PACKET_PLACEHOLDER",
  "FUTURE_REQUEST_TYPE_CLARIFICATION_CONTROLLED_ONE_RUN_PREP_PLACEHOLDER"
] as const;

console.log("=== v19.3 Owner Future Request Type Clarification Packet Validation ===\n");
...
ok(
  "doc/fixture identify v19.3 owner future request type clarification packet correctly",
  /v19\.3 - Owner Future Request Type Clarification Packet/.test(doc) &&
    fixtureRaw.includes("\"version\": \"v19.3\"") &&
    fixtureRaw.includes("\"executionType\": \"owner-future-request-type-clarification-packet only\"")
);

ok(
  "v19.3 scope status flags appear",
...
if (packageParsed && typeof packageParsed === "object") {
  ...
  ok("package has test:v19.3 script", scripts["test:v19.3"] === "tsx scripts/test-v193-owner-future-request-type-clarification-packet.mts");
}
...
if (fixtureParsed && typeof fixtureParsed === "object") {
  ...
  const boolKeys = [
    "clarificationOnly",
    "futureRequestTypeClarificationOnly",
    "reviewOnly",
    "placeholderOnly",
    "documentationOnly",
    "fixtureOnly",
  ...
  ok(
    "baseline confirms v13-v19.2 carry-forward",
    baseline.v13 === "CLOSED" &&
      baseline.v14 === "CLOSED" &&
      baseline.v15 === "CLOSED" &&
  ...
  const categories = asRecord(root.futureRequestTypeClarificationPlaceholders);
  ok("future request type clarification placeholders exist", clarificationCategories.every((key) => typeof categories[key] === "object"));
  ok(
    "no clarification category selected and placeholders only",
    clarificationCategories.every((key) => {
      const item = asRecord(categories[key]);
      return (
        item.placeholderClarificationOnly === true &&
        item.reviewClarificationOnly === true &&
        item.selected === false &&
  ...
  const req = asRecord(root.clarificationSeparationRequirements);
  ok(
    "clarification separation requirements exist",
...
ok(
  ...
  hasEveryLine(doc, [
    "v19.3 เป็นเอกสารอธิบายความหมายของประเภทคำขออนาคตเท่านั้น ใช้ให้ลุงเด่นเข้าใจความต่างของแต่ละทางเลือก แต่ยังไม่เลือกประเภทคำขอ ยังไม่ใช่ GO ยังไม่ใช่ approval และยังไม่ใช่ execution",
    "v19.3 ยังไม่ใส่ค่าจริง ยังไม่ขอค่าจริง และยังไม่แตะของจริง หากจะเลือกประเภทคำขอจริงในขั้นถัดไป ต้องเปิดงานใหม่แยกพร้อม prerequisite, exact command, exact target, expected evidence และ fresh owner approval ก่อนเสมอ"
  ])
);
...
ok(
  ...
  hasEveryLine(doc, [
    ...
    "safe default is hold/no action",
    "v19.3 is not approval",
    "v19.3 is not GO",
    "v19.3 is not execution",
    "v19.3 is not real-value insertion",
    "v19.3 is separated from v19.2/v19.1/v19.0/v18.11/v18.10A/v18.10/v18.9/v18.8/v18.7/v18.6A/v18.6/v18.5/v18.4/v18.3/v18.2/v18.1/v18.0/v17"
  ])
);

ok(
  "final decision wording exists",
  doc.includes("V19.3 OWNER FUTURE REQUEST TYPE CLARIFICATION PACKET CLOSED - READY FOR OWNER REVIEW ONLY / NO EXECUTION") &&
    fixtureRaw.includes("\"finalDecision\": \"V19.3 OWNER FUTURE REQUEST TYPE CLARIFICATION PACKET CLOSED - READY FOR OWNER REVIEW ONLY / NO EXECUTION\"")
);

console.log(`\nDone v19.3 validation - ${pass} PASS, ${fail} FAIL.\n`);

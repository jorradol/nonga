/**
 * v17.2 future real-value request draft template validator
 * Static checks only. Template-only and placeholder-only.
 *
 * npm run test:v17.2
 */
import { existsSync, readFileSync } from "node:fs";

const DOC_PATH = "docs/v17.2-future-real-value-request-draft-template.md";
const FIXTURE_PATH = "docs/examples/v17.2-future-real-value-request-draft-template.synthetic.json";
...
console.log("=== v17.2 Future Real-Value Request Draft Template Validation ===\n");
...
ok(
  "doc/fixture identify v17.2 correctly",
  /v17\.2 — Future Real-Value Request Draft Template/.test(doc) &&
    fixtureRaw.includes("\"version\": \"v17.2\"") &&
    fixtureRaw.includes("\"executionType\": \"future-real-value-request-draft-template only\"")
);

ok(
  "status is template-only no-execution placeholder-only not-owner-approval",
  hasEveryLine(doc, [
...
if (packageParsed && typeof packageParsed === "object") {
  ...
  ok(
    "package has test:v17.2 script",
    scripts["test:v17.2"] === "tsx scripts/test-v172-future-real-value-request-draft-template.mts"
  );
}
...
if (fixtureParsed && typeof fixtureParsed === "object") {
  ...
  ok(
    ...
    baseline["v13"] === "CLOSED" &&
      ...
      baseline["v17.2"] === "FUTURE REAL-VALUE REQUEST DRAFT TEMPLATE ONLY / NO EXECUTION"
  );

  const templateFields = asRecord(root.templateRequiredFields);
  ok(
    "template required fields complete with placeholder-only values",
    templateFields.futureOwnerReviewStatus === "FUTURE_OWNER_REVIEW_PLACEHOLDER_ONLY" &&
      templateFields.futureOwnerApprovalStatus === "FUTURE_OWNER_APPROVAL_PLACEHOLDER_ONLY" &&
      templateFields.exactCommand === "EXACT_COMMAND_PLACEHOLDER_ONLY" &&
      templateFields.exactTarget === "EXACT_TARGET_PLACEHOLDER_ONLY" &&
      templateFields.exactExpectedEvidence === "EXACT_EXPECTED_EVIDENCE_PLACEHOLDER_ONLY" &&
      templateFields.executionCountLimit === "EXECUTION_COUNT_LIMIT_PLACEHOLDER_ONLY" &&
      templateFields.oneRunOnly === "ONE_RUN_ONLY_PLACEHOLDER_ONLY" &&
      templateFields.automaticRetryPolicy === "NO_AUTOMATIC_RETRY_PLACEHOLDER_ONLY" &&
      templateFields.secondRunPolicy === "NO_SECOND_RUN_PLACEHOLDER_ONLY" &&
      templateFields.rollbackOwner === "ROLLBACK_OWNER_PLACEHOLDER_ONLY" &&
      templateFields.killSwitchOwner === "KILL_SWITCH_OWNER_PLACEHOLDER_ONLY" &&
      templateFields.stopConditions === "STOP_CONDITIONS_PLACEHOLDER_ONLY" &&
      templateFields.sanitizedEvidenceBundle === "SANITIZED_EVIDENCE_BUNDLE_PLACEHOLDER_ONLY" &&
      templateFields.repoHeadCheck === "REPO_HEAD_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.originHeadCheck === "ORIGIN_HEAD_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.workingTreeCheck === "WORKING_TREE_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.requiredTests === "REQUIRED_TESTS_PLACEHOLDER_ONLY" &&
      templateFields.publicProductionBoundaryCheck ===
        "PUBLIC_PRODUCTION_BOUNDARY_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.realLeadBoundaryCheck === "REAL_LEAD_BOUNDARY_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.realDealerBoundaryCheck === "REAL_DEALER_BOUNDARY_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.runtimeProviderGeminiBoundaryCheck ===
        "RUNTIME_PROVIDER_GEMINI_BOUNDARY_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.liveEndpointManualGuessBoundaryCheck ===
        "LIVE_ENDPOINT_MANUAL_GUESS_BOUNDARY_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.tokenSecretPiiBoundaryCheck === "TOKEN_SECRET_PII_BOUNDARY_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.thorDealerImportBoundaryCheck === "THOR_DEALER_IMPORT_BOUNDARY_CHECK_PLACEHOLDER_ONLY" &&
      templateFields.ownerFinalConfirmationBlock === "OWNER_FINAL_CONFIRMATION_PLACEHOLDER_ONLY"
  );

  const placeholders = asRecord(root.requiredPlaceholderValues);
  ok(
    "required placeholder values exact",
    placeholders.futureOwnerReview === "FUTURE_OWNER_REVIEW_PLACEHOLDER_ONLY" &&
      placeholders.futureOwnerApproval === "FUTURE_OWNER_APPROVAL_PLACEHOLDER_ONLY" &&
      placeholders.exactCommand === "EXACT_COMMAND_PLACEHOLDER_ONLY" &&
      placeholders.exactTarget === "EXACT_TARGET_PLACEHOLDER_ONLY" &&
      placeholders.exactExpectedEvidence === "EXACT_EXPECTED_EVIDENCE_PLACEHOLDER_ONLY" &&
      placeholders.executionCountLimit === "EXECUTION_COUNT_LIMIT_PLACEHOLDER_ONLY" &&
      placeholders.rollbackOwner === "ROLLBACK_OWNER_PLACEHOLDER_ONLY" &&
      placeholders.killSwitchOwner === "KILL_SWITCH_OWNER_PLACEHOLDER_ONLY" &&
      placeholders.stopConditions === "STOP_CONDITIONS_PLACEHOLDER_ONLY" &&
      placeholders.sanitizedEvidenceBundle === "SANITIZED_EVIDENCE_BUNDLE_PLACEHOLDER_ONLY" &&
      placeholders.repoHeadCheck === "REPO_HEAD_CHECK_PLACEHOLDER_ONLY" &&
      placeholders.originHeadCheck === "ORIGIN_HEAD_CHECK_PLACEHOLDER_ONLY" &&
      placeholders.workingTreeCheck === "WORKING_TREE_CHECK_PLACEHOLDER_ONLY" &&
      placeholders.requiredTests === "REQUIRED_TESTS_PLACEHOLDER_ONLY" &&
      placeholders.ownerFinalConfirmation === "OWNER_FINAL_CONFIRMATION_PLACEHOLDER_ONLY"
  );

  ok(
    "what v17.2 does not approve list complete",
    Array.isArray(root.whatV172DoesNotApprove) && root.whatV172DoesNotApprove.length >= 19
  );

  const freshApproval = asRecord(root.freshOwnerApprovalSeparation);
  ok(
    "fresh owner approval separation complete and true",
    Object.keys(freshApproval).length >= 7 &&
  ...
  ok(
    ...
    Object.keys(guard).length >= 19 && Object.values(guard).every((value) => value === false)
  );
  ...
  ok(
    ...
    Array.isArray(root.safeNegativeProhibitionContext) &&
      ...
      root.safeNegativeProhibitionContext.includes("this template does not authorize execution")
  );

  ok(
    "owner-friendly thai summary includes required lines",
    Array.isArray(root.ownerFriendlyThaiSummary) &&
      root.ownerFriendlyThaiSummary.includes("v17.2 เป็นแม่แบบสำหรับคำขอในอนาคตเท่านั้น") &&
  ...
  ok(
    "template review outcome options complete and non-authorizing",
    Array.isArray(root.templateReviewOutcomeOptions) &&
      root.templateReviewOutcomeOptions.includes(
        "TEMPLATE REVIEW COMPLETE — READY TO CONSIDER FUTURE REAL-VALUE INSERTION REQUEST ONLY"
      ) &&
      root.templateReviewOutcomeOptions.includes("HOLD TEMPLATE REVIEW — CLARIFICATION NEEDED") &&
      root.templateReviewOutcomeOptions.includes("REVISE TEMPLATE — DOCS/TESTS/FIXTURES ONLY") &&
      root.templateReviewOutcomeOptions.includes(
        "STOP BEFORE REAL-VALUE INSERTION — NO-GO TEMPLATE OUTCOME"
      ) &&
      root.templateOutcomeOptionsDoNotAuthorizeRealValueInsertionExecutionOrGo === true
  );
  ...
  ok(
    "final decision is template closure only",
    root.finalDecision ===
      "V17.2 TEMPLATE CLOSED — READY TO CONSIDER FUTURE REAL-VALUE INSERTION REQUEST ONLY / NO EXECUTION"
  );
}

ok(
  "doc includes template required field names",
  hasEveryLine(doc, [
    "futureOwnerReviewStatus",
    "futureOwnerApprovalStatus",
    "exactCommand",
    "exactTarget",
    ...
    "workingTreeCheck",
    "requiredTests",
    "publicProductionBoundaryCheck",
    "realLeadBoundaryCheck",
    "realDealerBoundaryCheck",
    "runtimeProviderGeminiBoundaryCheck",
    "liveEndpointManualGuessBoundaryCheck",
    "tokenSecretPiiBoundaryCheck",
    "thorDealerImportBoundaryCheck",
    "ownerFinalConfirmationBlock"
  ])
);
...
ok(
  "doc includes what v17.2 does not approve list",
  hasEveryLine(doc, [
    "GO",
    "execution",
    "one-run",
...
ok(
  "doc includes template review outcome options and non-authorizing statement",
  hasEveryLine(doc, [
    "TEMPLATE REVIEW COMPLETE — READY TO CONSIDER FUTURE REAL-VALUE INSERTION REQUEST ONLY",
...
console.log(`\nDone v17.2 validation - ${pass} PASS, ${fail} FAIL.\n`);

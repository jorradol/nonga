/**
 * v19.1 owner future request review packet validator
 * Static checks only. Review-packet-only, future-request-review-only, placeholder-only.
 *
 * npm run test:v19.1
 */
import { existsSync, readFileSync } from "node:fs";

const DOC_PATH = "docs/v19.1-owner-future-request-review-packet.md";
const FIXTURE_PATH = "docs/examples/v19.1-owner-future-request-review-packet.synthetic.json";
...
const requiredSections = [
  ...
  "currentStatusAfterV190Section",
  "closedBaselineCarryForwardSection",
  "v191ScopeSection",
  "reviewPacketOnlyReminderSection",
  "futureRequestReviewOnlyReminderSection",
  ...
  "v19ReviewPacketPurposeSection",
  "futureRequestCategoryReviewSection",
  "futureRequestCategoryNotSelectionSection",
  "ownerDecisionNotIncludedSection",
  "noFuturePathSelectedSection",
  "requestTypeReviewOnlyPlaceholderSection",
  "requestTypeExecutionDraftPlaceholderSection",
  "requestTypeRealValueInsertionDraftPlaceholderSection",
  "requestTypeOwnerApprovalPacketPlaceholderSection",
  "requestTypeControlledOneRunPrepPlaceholderSection",
  "prerequisiteReviewSection",
  "prerequisiteNotProvidedSection",
...
const categoryPlaceholders = [
  "FUTURE_REQUEST_CATEGORY_REVIEW_ONLY_PLACEHOLDER",
  "FUTURE_REQUEST_CATEGORY_EXECUTION_REQUEST_DRAFT_PLACEHOLDER",
  "FUTURE_REQUEST_CATEGORY_REAL_VALUE_INSERTION_DRAFT_PLACEHOLDER",
  "FUTURE_REQUEST_CATEGORY_OWNER_APPROVAL_PACKET_PLACEHOLDER",
  "FUTURE_REQUEST_CATEGORY_CONTROLLED_ONE_RUN_PREP_PLACEHOLDER"
] as const;

console.log("=== v19.1 Owner Future Request Review Packet Validation ===\n");
...
ok(
  "doc/fixture identify v19.1 owner future request review packet correctly",
  /v19\.1 - Owner Future Request Review Packet/.test(doc) &&
    fixtureRaw.includes("\"version\": \"v19.1\"") &&
    fixtureRaw.includes("\"executionType\": \"owner-future-request-review-packet only\"")
);

ok(
  "doc/fixture include review-packet-only future-request-review-only placeholder-only no-approval no-go no-execution",
...
if (packageParsed && typeof packageParsed === "object") {
  ...
  ok("package has test:v19.1 script", scripts["test:v19.1"] === "tsx scripts/test-v191-owner-future-request-review-packet.mts");
}
...
if (fixtureParsed && typeof fixtureParsed === "object") {
  ...
  ok(
    "baseline confirms v13-v19.0 carry-forward",
    baseline.v13 === "CLOSED" &&
      baseline.v14 === "CLOSED" &&
      baseline.v15 === "CLOSED" &&
  ...
  const categories = asRecord(root.futureRequestCategoryPlaceholders);
  ok("future request category placeholders exist", categoryPlaceholders.every((key) => typeof categories[key] === "object"));
  ok(
    "no category selected and categories are placeholder-only",
    categoryPlaceholders.every((key) => {
  ...
  ok(
    ...
    req.ownerDecisionNotIncluded === "OWNER_DECISION_NOT_INCLUDED_PLACEHOLDER_ONLY" &&
      ...
      req.prerequisiteReview === "PREREQUISITE_REVIEW_PLACEHOLDER_ONLY" &&
      req.prerequisiteNotProvided === "PREREQUISITE_NOT_PROVIDED_PLACEHOLDER_ONLY" &&
...
ok("doc includes final decision wording", doc.includes("V19.1 OWNER FUTURE REQUEST REVIEW PACKET CLOSED - READY FOR OWNER REVIEW ONLY / NO EXECUTION"));

console.log(`\nDone v19.1 validation - ${pass} PASS, ${fail} FAIL.\n`);

 ...
 *
 * npm run test:v106:manual-smoke-pre-run-gate
 */
import { readFileSync } from "node:fs";

const DOC_PATH = "docs/v10.6-admin-only-real-gemini-manual-smoke-pre-run-gate.md";
const SELF_PATH = "scripts/test-v106-admin-only-real-gemini-manual-smoke-pre-run-gate.mts";
const PKG_PATH = "package.json";
const NPM_SCRIPT_KEY = "test:v106:manual-smoke-pre-run-gate";
...
console.log("=== v10.6 Admin-Only Real Gemini Manual Smoke Pre-Run Gate Validation ===\n");
...
ok("doc contains v10.6 label", /v10\.6/i.test(doc));
...
const REQUIRED_DOC_PHRASES: Array<[string, RegExp]> = [
  ...
  ["one admin-only manual smoke run", /one admin-only manual smoke run/i],
  ...
  ["eligible for separate execution request only", /eligible for separate execution request only/i],
  ["v10.7 Admin-Only Real Gemini Manual Smoke Execution Request", /v10\.7 Admin-Only Real Gemini Manual Smoke Execution Request/i],
  ["no normal test hitting Gemini", /no normal test hitting Gemini/i],
];
...
ok("package includes v10.6 script key", pkg.includes(`"${NPM_SCRIPT_KEY}"`));
ok(
  "package points to v10.6 validator script",
  pkg.includes("scripts/test-v106-admin-only-real-gemini-manual-smoke-pre-run-gate.mts")
);

{
  const head = self.split('ok("package includes v10.6 script key"')[0] ?? self;
...
console.log(`\nDone v10.6 manual-smoke-pre-run-gate validation - ${pass} PASS, ${fail} FAIL.\n`);

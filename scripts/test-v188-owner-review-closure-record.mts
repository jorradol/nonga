/**
 * v18.8 owner review closure record validator
 * Static checks only. Closure-record-only, review-only, no-execution, placeholder-only.
 *
 * npm run test:v18.8
 */
import { existsSync, readFileSync } from "node:fs";

const DOC_PATH = "docs/v18.8-owner-review-closure-record.md";
const FIXTURE_PATH = "docs/examples/v18.8-owner-review-closure-record.synthetic.json";
...
console.log("=== v18.8 Owner Review Closure Record Validation ===\n");
...
ok(
  "doc/fixture identify v18.8 owner review closure record correctly",
  /v18\.8 — Owner Review Closure Record/.test(doc) &&
    fixtureRaw.includes("\"version\": \"v18.8\"") &&
    fixtureRaw.includes("\"executionType\": \"owner-review-closure-record only\"")
);

ok(
  "doc/fixture include closure-record-only review-only no-approval no-go no-execution placeholder-only",
...
ok(
  "baseline confirms v13-v17 and v18.0-v18.7 closures",
  hasEveryLine(doc, [
    "v13 = CLOSED",
    "v14 = CLOSED",
    "v15 = CLOSED",
...
const requiredSections = [
  ...
  "v18ReviewPackageClosurePurposeSection",
  ...
  "v18ReviewPackageStatusSection",
  "ownerReviewReadyOnlySection",
  "ownerDecisionNotIncludedSection",
  "noFuturePathSelectedSection",
  "noApprovalGrantedSection",
  "noExecutionRequestCreatedSection",
  "noRealValueInsertionCreatedSection",
...
const requiredClosureReferenceLines = [
...
ok("doc includes required closure reference lines", hasEveryLine(doc, requiredClosureReferenceLines));
...
if (packageParsed && typeof packageParsed === "object") {
  ...
  ok("package has test:v18.8 script", scripts["test:v18.8"] === "tsx scripts/test-v188-owner-review-closure-record.mts");
}
...
if (fixtureParsed && typeof fixtureParsed === "object") {
  ...
  ok(
    "v18.8 separation flags true",
    separation.v188SeparatedFromV187 === true &&
      separation.v188SeparatedFromV186A === true &&
      separation.v188SeparatedFromV186 === true &&
      separation.v188SeparatedFromV185 === true &&
      separation.v188SeparatedFromV184 === true &&
      separation.v188SeparatedFromV183 === true &&
      separation.v188SeparatedFromV182 === true &&
      separation.v188SeparatedFromV181 === true &&
      separation.v188SeparatedFromV180 === true &&
      separation.v188SeparatedFromV17 === true &&
      separation.v188DoesNotApproveExecution === true &&
      separation.v188DoesNotApproveGo === true &&
      separation.v188DoesNotApproveRealValueInsertion === true
  );

  ok(
    "closure references exist and are placeholder-only non-authorizing",
    Array.isArray(root.closureReferences) &&
      root.closureReferences.length === 9 &&
      root.closureReferences.every((entry: unknown) => {
        const e = asRecord(entry);
        return (
          typeof e.entryName === "string" &&
          e.placeholderOnly === true &&
          e.closureRecordReferenceOnly === true &&
          e.notApproval === true &&
          e.notExecution === true &&
          e.notGo === true &&
  ...
  const ready = asRecord(root.ownerReviewReadyOnly);
  ok("owner review ready only exists", ready.placeholder === "OWNER_REVIEW_READY_ONLY_PLACEHOLDER_ONLY" && ready.exists === true);
  ...
  const noPath = asRecord(root.noFuturePathSelected);
  ok("no future path selected", noPath.placeholder === "NO_FUTURE_PATH_SELECTED_PLACEHOLDER_ONLY" && noPath.notSelected === true);

  const noApproval = asRecord(root.noApprovalGranted);
  ok("no approval granted", noApproval.placeholder === "NO_APPROVAL_GRANTED_PLACEHOLDER_ONLY" && noApproval.granted === false);

  const noExecutionReq = asRecord(root.noExecutionRequestCreated);
  ok(
    "no execution request created",
    noExecutionReq.placeholder === "NO_EXECUTION_REQUEST_CREATED_PLACEHOLDER_ONLY" && noExecutionReq.created === false
  );

  const noRealInsert = asRecord(root.noRealValueInsertionCreated);
  ok(
    "no real-value insertion created",
    noRealInsert.placeholder === "NO_REAL_VALUE_INSERTION_CREATED_PLACEHOLDER_ONLY" && noRealInsert.created === false
  );
...
> = [
  ...
  [
    "future path selected wording",
    /\b(?:future\s+path\s+selected|selected\s+future\s+path)\b/i,
    { allowSafeNegativeContext: true }
  ]
];
...
ok(
  "doc carries required closure summary reminders",
  hasEveryLine(doc, [
...
ok(
  ...
  doc.includes("V18.8 OWNER REVIEW CLOSURE RECORD CLOSED — READY FOR OWNER REVIEW ONLY / NO EXECUTION")
);

console.log(`\nDone v18.8 validation - ${pass} PASS, ${fail} FAIL.\n`);

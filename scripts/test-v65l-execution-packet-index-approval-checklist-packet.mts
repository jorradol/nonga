/**
 * v6.5L — Execution Packet Index & Approval Checklist Packet (static validation only)
...
const AUTO_EXEC_AUTH_PATTERNS = [
  ...
  /v6\.5L\s+approves\s+production\s+pilot\s+automatically/i,
  /v6\.5L\s+authorizes\s+production\s+pilot\s+start/i,
  /v6\.5L\s+captures\s+approval\s+in\s+v6\.5L\s+slice/i,
...
const BAD_APPROVED_PATTERNS = [
  ...
  /May Execute from v6\.5L\?\s*=\s*Yes/i,
  ...
  /execution\s+authorized\s+by\s+v6\.5L/i,
];

const HEAD_SHA = "0e3ee2cf7fa855ac25970ec46877d163545c8f99";
...
const selfSrc = readFileSync(
  "scripts/test-v65l-execution-packet-index-approval-checklist-packet.mts",
  "utf8"
);
...
{
  ok("v6.5L doc exists", doc.length > 12000);
  ok("doc v6.5L label", doc.includes("v6.5L"));
...
// --- safety positioning ---
{
  ok("doc HEAD 0e3ee2c", doc.includes(HEAD_SHA) || doc.includes("0e3ee2c"));
  ...
  ok(
    "doc index checklist packet only",
    /index\/checklist|Execution Packet Index/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc may execute from v65l is no",
    /May Execute from v6\.5L\?\s*=\s*No|May Execute from v6\.5L\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
...
// --- required sections ---
{
  ...
  ok(
    "doc what v65l is is not",
    /What v6\.5L Is \/ Is Not/i.test(doc)
  );
  ok(
    "doc packet index principle",
    /Packet Index Principle/i.test(doc)
  );
  ok(
    "doc approval checklist principle",
    /Approval Checklist Principle/i.test(doc)
  );
  ok("doc packet inventory", /Packet Inventory/i.test(doc));
  ok("doc execution gate index", /Execution Gate Index/i.test(doc));
  ok(
    "doc readiness policy packet index",
    /Readiness \/ Policy Packet Index/i.test(doc)
  );
  ok(
    "doc approval boundary packet index",
    /Approval Boundary Packet Index/i.test(doc)
  );
  ok(
    "doc packet-to-gate mapping",
    /Packet-to-Gate Mapping/i.test(doc)
  );
  ok(
    "doc gate-to-approval checklist",
    /Gate-to-Approval Checklist/i.test(doc)
  );
  ok(
    "doc gate-to-evidence checklist",
    /Gate-to-Evidence Checklist/i.test(doc)
  );
  ok(
    "doc gate-to-report checklist",
    /Gate-to-Report Checklist/i.test(doc)
  );
  ok(
    "doc pre-commit commit push deploy checklist separation",
    /Pre-Commit \/ Commit \/ Push \/ Deploy Checklist Separation/i.test(doc)
  );
  ok(
    "doc runtime deploy production secret gemini approval checklist",
    /Runtime \/ Deploy \/ Production \/ Secret \/ Gemini Approval Checklist/i.test(
      doc
    )
  );
  ...
  ok(
    "doc missing packet handling",
    /Missing Packet Handling/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc checklist completion rules",
    /Checklist Completion Rules/i.test(doc)
  );
  ok(
    "doc packet status taxonomy",
    /Packet Status Taxonomy/i.test(doc)
  );
  ok(
    "doc execution readiness status taxonomy",
    /Execution Readiness Status Taxonomy/i.test(doc)
  );
  ok(
    "doc owner review checklist",
    /Owner Review Checklist/i.test(doc)
  );
  ok("doc reviewer checklist", /Reviewer Checklist/i.test(doc));
  ok(
    "doc future execution packet naming rules",
    /Future Execution Packet Naming Rules/i.test(doc)
  );
  ok(
    "doc future execution packet required sections",
    /Future Execution Packet Required Sections/i.test(doc)
  );
  ok(
    "doc future execution packet required reports",
    /Future Execution Packet Required Reports/i.test(doc)
  );
  ok(
    "doc version boundary carry-forward rules",
    /Version Boundary Carry-Forward Rules/i.test(doc)
  );
  ok(
  ...
  ok("doc packet index table", /Packet Index Table/i.test(doc));
  ok(
    "doc gate-to-approval checklist table",
    /Gate-to-Approval Checklist Table/i.test(doc)
  );
  ok(
    "doc index checklist only not approval",
    /index\/checklist only|index\/checklist artifact/i.test(doc)
  );
}

// --- packet index table rows ---
{
...
// --- gate-to-approval checklist table rows ---
{
  ok(
...
// --- no secrets / PII / sensitive samples ---
{
  ...
  ok(
    "doc v65l does not authorize execution alone",
    /May Execute from v6\.5L\?\s*=\s*No|Execution Allowed\?\s*=\s*No/i.test(doc)
  );
...
// --- package script ---
{
  ok(
    "package v65l script",
    pkg.includes("test:v65l-execution-packet-index-approval-checklist-packet")
  );
  ok(
...
console.log(
  "\nDone v6.5L Execution Packet Index & Approval Checklist Packet tests."
);

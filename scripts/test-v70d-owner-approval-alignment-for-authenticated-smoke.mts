 ...
 * npm run test:v70d-owner-approval-alignment-for-authenticated-smoke
 *
...
const DOC_PATH =
  "docs/v7.0D-owner-approval-alignment-for-authenticated-smoke.md";
...
ok("phase name v7.0D", /v7\.0D/i.test(doc));
ok(
  "phase title owner approval alignment",
  /Owner Approval Alignment for Authenticated Smoke/i.test(doc)
);
ok(
  "branch feature/chat-image-attachment-v1",
  doc.includes("feature/chat-image-attachment-v1")
);
ok("baseline 6a861ea", doc.includes("6a861ea"));

// --- verdict ---
ok(
  "verdict OWNER APPROVAL ALIGNMENT ONLY",
  /PARTIAL — OWNER APPROVAL ALIGNMENT ONLY|PARTIAL -- OWNER APPROVAL ALIGNMENT ONLY/i.test(
    doc
  )
);
...
ok(
  "why owner approval alignment section",
  /Why owner approval alignment is needed/i.test(doc)
);
ok("approval scope section", /##\s*\d+\.\s*Approval scope/i.test(doc));
ok("explicit non-approvals section", /Explicit non-approvals/i.test(doc));
ok(
  "authenticated smoke permission boundary section",
  /Authenticated smoke permission boundary/i.test(doc)
);
ok(
  "operator safe-session dependency section",
  /Operator safe-session dependency/i.test(doc)
);
ok(
  "non-pii evidence dependency section",
  /Non-PII evidence dependency/i.test(doc)
);
...
ok(
  "what owner approval can allow section",
  /What owner approval can allow/i.test(doc)
);
ok(
  "what owner approval cannot allow section",
  /What owner approval cannot allow/i.test(doc)
);
ok(
...
// --- approval scope / non-approval items ---
for (let i = 1; i <= 4; i++) {
  ok(`approval scope AS-${i}`, doc.includes(`AS-${i}`));
}
for (let i = 1; i <= 7; i++) {
...
ok("boundary no credential return", /ห้ามส่งกลับมา/i.test(doc));

// --- non-PII evidence dependency / count-status ---
ok("count_only present", /count-only/i.test(doc));
ok("status_only present", /status-only/i.test(doc));

// --- v7.0D is NOT authenticated smoke execution ---
ok(
  "not authenticated smoke execution",
  /ไม่ใช่.*authenticated smoke execution|NOT an authenticated smoke execution|v7\.0D \*\*ไม่ใช่\*\* authenticated smoke execution/i.test(
    doc
  )
);
...
ok(
  ...
  pkg.includes(
    "test:v70d-owner-approval-alignment-for-authenticated-smoke"
  )
);

console.log(
  "\nDone v7.0D owner approval alignment for authenticated smoke tests.\n"
);

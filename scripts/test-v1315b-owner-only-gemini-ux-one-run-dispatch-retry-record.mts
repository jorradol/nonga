/**
 * v13.15B owner-only Gemini UX one-run dispatch retry validator
 * Static checks only. No runtime mutation, no endpoint/provider call.
 *
 * npm run test:v13.15B
 */
import { existsSync, readFileSync } from "node:fs";

const DOC_PATH =
  "docs/v13.15B-owner-only-gemini-ux-one-run-dispatch-retry-record.md";
const SELF_PATH =
  "scripts/test-v1315b-owner-only-gemini-ux-one-run-dispatch-retry-record.mts";
...
console.log("=== v13.15B Owner-Only Gemini UX Dispatch Retry Validation ===\n");
...
ok("doc has substantial content", doc.length > 7000, `${doc.length} chars`);

const executionChecks: Array<[string, RegExp]> = [
  [
    "owner-only dispatch retry yes",
    /owner-only Gemini UX dispatch retry:\s*yes/i,
  ],
...
ok("baseline includes edc90e1", /baseline commit.*edc90e1/i.test(doc));
ok("root cause carried from v13.15A", /Root cause carried from v13\.15A/i.test(doc));
ok("mentions ai_first_disabled root cause", /ai_first_disabled/i.test(doc));
...
ok(
  "owner-only ai-first user-visible evidence section",
  /Owner-only \/ AI-first \/ user-visible gate evidence/i.test(doc)
);
ok(
...
const finalAllowed = [
  ...
  "HOLD — unsafe Gemini UX dispatch boundary detected",
];
...
const unsafeClaims: Array<[string, RegExp]> = [
  ...
  [
    "production enabled",
    /\bproduction\b.*\b(opened|enabled|active|released|live)\b/i,
  ],
  [
    "buyer-facing enabled",
    /\bbuyer-facing\b.*\b(opened|enabled|active|released|live)\b/i,
  ],
...
ok(
  "no claim gemini dispatch succeeded",
  !/\bGemini dispatch (?:success|successful|สำเร็จแล้ว)\b/i.test(doc)
);
ok("validator includes secret scan", /forbiddenValuePatterns/.test(self));

console.log(`\nDone v13.15B validation - ${pass} PASS, ${fail} FAIL.\n`);

/**
 * v6.3B.5A — Staging Hosting Deploy + Manual Smoke Execution Record (static validation only)
 * npm run test:v63b5a-staging-hosting-deploy-execution-record
 */
import { readFileSync } from "node:fs";

const DOC_PATH =
  "docs/v6.3B.5A-staging-hosting-deploy-execution-record.md";
const V635_DOC = "docs/v6.3B.5-in-chat-curated-analysis-sales-copy-weave.md";
...
const CODE_SHA = "e98ac1ce8e3895c8a489b6a1db5a7fecc5cc9243";
const LIVE_JS = "index-CPoVE1cX.js";
const PREV_JS = "index-tLeqTuup.js";
...
console.log(
  "=== v6.3B.5A Staging Hosting Deploy + Manual Smoke Execution Record ===\n"
);

const doc = readFileSync(DOC_PATH, "utf8");
const docLower = doc.toLowerCase();
const selfSrc = readFileSync(
  "scripts/test-v63b5a-staging-hosting-deploy-execution-record.mts",
  "utf8"
);
const pkg = readFileSync("package.json", "utf8");
const v635Doc = readFileSync(V635_DOC, "utf8");
...
// --- doc exists + v6.3B.5A record ---
{
  ok("execution doc exists", doc.length > 6000);
  ok("doc v6.3B.5A label", doc.includes("v6.3B.5A"));
  ok(
  ...
  ok("doc HEAD e98ac1c", doc.includes(CODE_SHA) || doc.includes("e98ac1c"));
  ok("doc references v635", /v6\.3B\.5/i.test(doc));
...
// --- preflight ---
{
  ...
  ok("preflight git e98ac1c", doc.includes("e98ac1c"));
}
...
// --- deploy summary ---
{
  ...
  ok("live bundle CPoVE1cX", doc.includes(LIVE_JS));
...
// --- manual smoke pass ---
{
  ...
  ok("ms-a03 featureWeave pass", /MS-A03.*PASS/i.test(doc) && /featureWeave|ช่วยขาย/i.test(doc));
  ok("ms-a04 sales tone pass", /MS-A04.*PASS/i.test(doc));
  ok("ms-a07 tts pass", /MS-A07.*PASS/i.test(doc));
  ok("ms-a08 no junk tokens", /MS-A08.*PASS/i.test(doc));
  ok("ms-a09 steering pass", /MS-A09.*PASS/i.test(doc) && doc.includes(STEERING_CANONICAL));
  ok("ms-c04 automated pass", /MS-C04.*PASS/i.test(doc) && doc.includes(LIVE_JS));
  ok("lung notes pass quality", /พอใช้และผ่าน|พอใช้/i.test(doc));
  ok("future tone refinement noted", /future refinement|Future refinement|เซลล์มือทอง/i.test(doc));
  ok("future not in slice", /not in v6\.3B\.5A|ไม่แก้ tone ใน record slice/i.test(doc));
}

// --- observation ---
{
  ok("observation section", /Observation.*v6\.3B\.5/i.test(doc));
  ok("observation in-chat weave contract", /featureWeave|in-chat weave/i.test(docLower));
...
// --- outcome safety ---
{
  ...
  ok("rollback tLeqTuup hosting", doc.includes(PREV_JS) && /hosting/i.test(docLower));
  ok("safety confirmations section", /Safety Confirmations/i.test(doc));
  ok("compliance section", /Compliance.*v6\.3B\.5A/i.test(doc));
}
...
// --- cross-ref v635 ---
{
  ok("v635 doc exists", v635Doc.includes("v6.3B.5"));
}
...
{
  ok(
    "package v635a execution record script",
    pkg.includes("test:v63b5a-staging-hosting-deploy-execution-record")
  );
  ok(
...
console.log(
  "\nDone v6.3B.5A Staging Hosting Deploy Execution Record tests."
);

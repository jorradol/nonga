/**
 * v6.6A — Controlled Revenue Pilot Scope Packet (static validation only)
 * npm run test:v66a-controlled-revenue-pilot-scope-packet
 */
...
const DOC_PATH = "docs/v6.6A-controlled-revenue-pilot-scope-packet.md";
const V65W_DOC =
...
const AUTO_EXEC_AUTH_PATTERNS = [
  ...
  /v6\.6A\s+captures\s+approval\s+for\s+revenue\s+pilot/i,
  /v6\.6A\s+authorizes\s+revenue\s+pilot\s+start/i,
  /v6\.6A\s+authorizes\s+dealer\s+onboarding/i,
  /v6\.6A\s+approves\s+payment\s+automation/i,
  /execution\s+approved\s+for\s+production/i,
  /deploy\s+approved\s+for\s+production/i,
  /Revenue Pilot is approved by v6\.6A alone/i,
  /v6\.6A\s+approved\b/i,
];

const BAD_REVENUE_PILOT_APPROVED_PATTERNS = [
  ...
  /Execution Allowed from v6\.6A\?\s*=\s*Yes/i,
  /Gemini Invocation in v6\.6A\?\s*=\s*Yes/i,
  ...
  /revenue pilot has been started/i,
  /dealer has been onboarded/i,
  /payment has been enabled/i,
  /execution authorized by v6\.6A/i,
];

const HEAD_SHA = "d05b82a2c14fc83c6b4cd18ff2a052a64c89a0b0";
...
console.log("=== v6.6A Controlled Revenue Pilot Scope Packet ===\n");
...
const selfSrc = readFileSync(
  "scripts/test-v66a-controlled-revenue-pilot-scope-packet.mts",
  "utf8"
);
...
{
  ok("v6.6A doc exists", doc.length > 8000);
  ok("doc v6.6A label", doc.includes("v6.6A"));
...
// --- safety positioning ---
{
  ok("doc HEAD d05b82a", doc.includes(HEAD_SHA) || doc.includes("d05b82a"));
  ok("doc staging bundle", doc.includes(STAGING_BUNDLE));
  ok("doc Real Gemini OFF", /Real Gemini.*OFF|OFF.*Real Gemini/i.test(doc));
  ok(
    "doc no revenue pilot start in v6.6A",
    /no revenue pilot start|Revenue Pilot Started\?\s*=\s*No|does not start revenue pilot/i.test(
      doc
    )
  );
  ok(
    "doc no Gemini invocation in v6.6A",
    /no Gemini invocation|Gemini Invocation in v6\.6A\?\s*=\s*No|does not invoke Gemini/i.test(
      doc
    )
  );
  ok(
...
// --- status fields ---
{
  ...
  ok(
    "doc gemini invocation in v66a is no",
    /Gemini Invocation in v6\.6A\?\s*=\s*No|Gemini Invocation in v6\.6A\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
  ...
  ok(
    "doc execution allowed from v66a is no",
    /Execution Allowed from v6\.6A\?\s*=\s*No|Execution Allowed from v6\.6A\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
    "doc does not state revenue pilot approved",
    /Does not state revenue pilot approved|does not state revenue pilot approved/i.test(
      doc
    )
  );
  ok(
...
// --- required sections ---
{
  ...
  ok(
    "doc current technical state summary",
    /Current Technical State Summary/i.test(doc)
  );
  ok("doc v6.5 closure summary", /v6\.5 Closure Summary/i.test(doc));
  ok("doc gate abcd carry-forward", /Gate A\/B\/C\/D Carry-Forward/i.test(doc));
  ok("doc why v6.6 exists", /Why v6\.6 Exists/i.test(doc));
  ok("doc revenue urgency context", /Revenue Urgency Context/i.test(doc));
  ok(
    "doc controlled revenue pilot goal",
    /Controlled Revenue Pilot Goal/i.test(doc)
  );
  ok("doc what v66a is is not", /What v6\.6A Is \/ Is Not/i.test(doc));
  ok("doc pilot customer profile", /Pilot Customer Profile/i.test(doc));
  ok("doc pilot size limit", /Pilot Size Limit/i.test(doc));
  ok("doc pilot feature scope", /Pilot Feature Scope/i.test(doc));
  ok("doc pilot feature non-scope", /Pilot Feature Non-Scope/i.test(doc));
  ...
  ok(
    "doc payment billing implementation exclusion",
    /Payment\/Billing Implementation Exclusion/i.test(doc)
  );
  ok("doc data handling boundary", /Data Handling Boundary/i.test(doc));
  ok("doc dealer data boundary", /Dealer Data Boundary/i.test(doc));
  ok("doc customer data boundary", /Customer Data Boundary/i.test(doc));
  ok("doc gemini cost boundary", /Gemini\/Cost Boundary/i.test(doc));
  ok("doc recommended revenue options", /Recommended Revenue Options/i.test(doc));
  ok("doc option comparison", /Option Comparison/i.test(doc));
  ok(
    "doc recommended first paid offer",
    /Recommended First Paid Offer/i.test(doc)
  );
  ok("doc suggested pricing bands", /Suggested Pricing Bands/i.test(doc));
  ...
  ok(
    "doc future v66 execution boundary",
    /Future v6\.6 Execution Boundary/i.test(doc)
  );
  ...
  ok(
    ...
    /อนุมัติให้เตรียม Controlled Revenue Pilot/i.test(doc)
  );
  ok("doc pricing hypothesis", /pricing hypothesis/i.test(doc));
  ok("doc option 1 setup fee", /Option 1.*Setup Fee|Setup Fee Pilot/i.test(doc));
  ok("doc option 2 monthly", /Option 2.*Monthly|Monthly Dealer Pilot/i.test(doc));
  ok(
    "doc option 3 listing content",
    /Option 3.*Listing|Listing \/ Content Package/i.test(doc)
  );
}

// --- no bad revenue pilot approved claims ---
{
  for (const pat of BAD_REVENUE_PILOT_APPROVED_PATTERNS) {
    ok(
      `doc no bad revenue pilot approved ${pat.source.slice(0, 22)}`,
      !pat.test(doc)
    );
  }
}
...
// --- no secrets / PII / sensitive samples ---
{
  ...
  ok(
    "doc v66a does not authorize execution alone",
    /Execution Allowed from v6\.6A\?\s*=\s*No/i.test(doc)
  );
...
// --- package script ---
{
  ok(
    "package v66a script",
    pkg.includes("test:v66a-controlled-revenue-pilot-scope-packet")
  );
  ok(
    "package points to mts",
    pkg.includes("scripts/test-v66a-controlled-revenue-pilot-scope-packet.mts")
  );
}

console.log("\nDone v6.6A Controlled Revenue Pilot Scope Packet tests.");

 ...
 * npm run test:v70t-operator-manual-smoke-instruction-sanitized-evidence-collection-guide
 *
 ...
 * Docs + static validation only. Verdict = INSTRUCTION ONLY / PENDING / WAITING.
 */
import { readFileSync } from "node:fs";

const DOC_PATH =
  "docs/v7.0T-operator-manual-smoke-instruction-sanitized-evidence-collection-guide.md";
...
ok("phase name v7.0T", /v7\.0T/i.test(doc));
ok(
  "phase title operator manual smoke instruction guide",
  /Operator Manual Smoke Instruction \/ Sanitized Evidence Collection Guide/i.test(
    doc
  )
);
ok(
...
ok("baseline 43ad97c", doc.includes("43ad97c"));

// --- verdict INSTRUCTION ONLY / PENDING / WAITING ---
ok(
  "verdict instruction only pending/waiting",
  /INSTRUCTION ONLY — PENDING \/ WAITING FOR OPERATOR EVIDENCE/i.test(doc)
);
ok(
  "current verdict field instruction only",
  /Current verdict[^\n]*INSTRUCTION ONLY — PENDING \/ WAITING FOR OPERATOR EVIDENCE/i.test(
    doc
  )
);
ok(
...
ok(
  "v7.0T does not record pass itself",
  /v7\.0T records PASS itself[^\n]*NO|ไม่บันทึก PASS เอง/i.test(doc)
);
...
ok("references v7.0S", /v7\.0S/i.test(doc));
...
ok("operator pre-flight checklist section", /Operator pre-flight checklist/i.test(doc));
ok("manual smoke steps section", /Manual smoke steps/i.test(doc));
ok(
  "sanitized evidence return template AS section",
  /Sanitized evidence return template — AS-01\.\.AS-09/i.test(doc)
);
ok(
  "sanitized evidence return template IN section",
  /Sanitized evidence return template — IN-1\.\.IN-10/i.test(doc)
);
...
ok("result boundary section", /Result boundary/i.test(doc));
ok(
...
// --- pre-flight checklist PF-1..PF-5 ---
ok("pre-flight PF present", /PF-1/.test(doc) && /PF-5/.test(doc));
ok("pre-flight staging only", /ยืนยันใช้ staging URL เท่านั้น/i.test(doc));
ok("pre-flight no production", /ยืนยันไม่เปิด production/i.test(doc));
ok(
  "pre-flight no credential to agent",
  /ยืนยันไม่ส่ง credential ให้ agent/i.test(doc)
);
ok(
  "pre-flight no session capture",
  /ยืนยันไม่ capture session\/cookie\/token/i.test(doc)
);
ok(
  "pre-flight no sensitive screenshot",
  /ยืนยันไม่ถ่าย screenshot ที่มีข้อมูลลับ/i.test(doc)
);

// --- manual smoke steps ST-1..ST-7 ---
ok("manual smoke steps ST present", /ST-1/.test(doc) && /ST-7/.test(doc));
ok("step open staging", /เปิด staging/i.test(doc));
ok("step manual login", /login เองด้วยวิธี manual/i.test(doc));
ok("step check landing/dashboard", /ตรวจ landing \/ dashboard/i.test(doc));
ok(
  "step check authenticated area",
  /ตรวจ dealer \/ authenticated area/i.test(doc)
);
ok(
  "step check lead queue no copy",
  /ไม่คัดลอกข้อมูลจริง/i.test(doc)
);
ok(
  "step check no cross-dealer",
  /ตรวจว่าไม่เห็น cross-dealer lead/i.test(doc)
);
ok("step logout cleanup", /logout \/ session cleanup/i.test(doc));

// --- AS-01..AS-09 return template each id ---
...
ok(
  "operator fills pass/partial/fail/abort/blocked",
  /PASS \/ PARTIAL \/ FAIL \/ ABORT \/ BLOCKED/i.test(doc)
);
...
ok("allowed example staging loaded", /"staging loaded"/i.test(doc));
ok(
  "allowed example manual login no credential shared",
  /"manual login completed, no credential shared"/i.test(doc)
);
ok("allowed example dashboard visible", /"dashboard visible"/i.test(doc));
ok(
...
ok(
  "forbidden example customer name/private details",
  /customer name or private customer details/i.test(doc)
);
ok(
...
ok(
  ...
  /ห้าม agent รับ credential\/password\/OTP\/session\/cookie\/token/i.test(doc)
);
ok(
...
ok(
  ...
  /operator\/lungden ต้อง login เองแบบ manual เท่านั้น/i.test(doc)
);
ok(
  "no send real customer data rule",
  /ห้ามส่งข้อมูลลูกค้าจริง/i.test(doc)
);

// --- abort/no-go conditions AB-1..AB-7 ---
ok("abort conditions AB present", /AB-1/.test(doc) && /AB-7/.test(doc));
ok(
  "abort credential/session exposure",
  /credential \/ session exposure/i.test(doc)
);
...
ok("abort cross-dealer visibility", /cross-dealer lead visibility/i.test(doc));
ok("abort wrong dealer data", /wrong dealer data \/ ข้อมูลผิด dealer/i.test(doc));
ok(
  "abort firestore write/mutation",
  /Firestore write \/ lead mutation/i.test(doc)
);
ok(
  "abort production/public/deploy/gemini/thor",
  /production \/ public signup \/ deploy \/ Gemini \/ Thor import action/i.test(
    doc
  )
);
ok(
  "abort sanitized on pii/lead/cross-dealer/session",
  /หากเจอ PII \/ lead leak \/ cross-dealer leak \/ session exposure[^\n]*หยุดทันที/i.test(
    doc
  )
);
...
// --- result boundary ---
ok(
  "v7.0T not record pass itself",
  /v7\.0T[^\n]*ไม่บันทึก PASS เอง/i.test(doc)
);
ok(
  "next slice closes result",
  /รอบถัดไปเท่านั้นที่จะนำ sanitized evidence มาปิด result/i.test(doc)
);
ok(
  "no evidence pending/waiting/blocked",
  /ถ้าไม่มี operator evidence ให้คง \*\*PENDING \/ WAITING\*\*|ไม่มี operator evidence[^\n]*PENDING \/ WAITING[^\n]*BLOCKED \/ NOT RUN/i.test(
    doc
  )
);
...
ok(
  ...
  pkg.includes(
    "test:v70t-operator-manual-smoke-instruction-sanitized-evidence-collection-guide"
  )
);

console.log(
  "\nDone v7.0T operator manual smoke instruction / sanitized evidence collection guide tests.\n"
);

/**
 * v6.5K — Execution Approval Separation & Version Boundary Packet (static validation only)
...
const AUTO_EXEC_AUTH_PATTERNS = [
  ...
  /v6\.5K\s+approves\s+production\s+pilot\s+automatically/i,
  /v6\.5K\s+authorizes\s+production\s+pilot\s+start/i,
  /v6\.5K\s+captures\s+approval\s+in\s+v6\.5K\s+slice/i,
  ...
  /v6\.5K\s+authorizes\s+secret\s+wiring\s+automatically/i,
  /production\s+pilot\s+is\s+approved/i,
];

const BAD_APPROVED_PATTERNS = [
  /Captured by v6\.5K\?\s*=\s*Yes/i,
  /Execution Allowed by v6\.5K\?\s*=\s*Yes/i,
  /May Execute Without Separate Approval\?\s*=\s*Yes/i,
  ...
  /execution\s+authorized\s+by\s+v6\.5K/i,
];

const HEAD_SHA = "5df748432d9c5cc521636066f05a61b70013373c";
...
const selfSrc = readFileSync(
  "scripts/test-v65k-execution-approval-separation-version-boundary-packet.mts",
  "utf8"
);
...
{
  ok("v6.5K doc exists", doc.length > 12000);
  ok("doc v6.5K label", doc.includes("v6.5K"));
...
// --- safety positioning ---
{
  ok("doc HEAD 5df7484", doc.includes(HEAD_SHA) || doc.includes("5df7484"));
  ...
  ok(
    "doc approval separation packet only",
    /approval separation packet|Execution Approval Separation/i.test(doc)
  );
  ok(
    "doc captured by v65k is no",
    /Captured by v6\.5K\?\s*=\s*No|Captured by v6\.5K\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
    "doc execution allowed by v65k is no",
    /Execution Allowed by v6\.5K\?\s*=\s*No|Execution Allowed by v6\.5K\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
    "doc may execute without separate approval is no",
    /May Execute Without Separate Approval\?\s*=\s*No|May Execute Without Separate Approval\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
...
// --- required sections ---
{
  ...
  ok(
    "doc what v65k is is not",
    /What v6\.5K Is \/ Is Not/i.test(doc)
  );
  ok(
    "doc approval separation principle",
    /Approval Separation Principle/i.test(doc)
  );
  ok(
    "doc version boundary principle",
    /Version Boundary Principle/i.test(doc)
  );
  ok("doc approval taxonomy", /Approval Taxonomy/i.test(doc));
  ok(
    "doc execution gate taxonomy",
    /Execution Gate Taxonomy/i.test(doc)
  );
  ok(
    "doc required separate approval list",
    /Required Separate Approval List/i.test(doc)
  );
  ok(
    "doc forbidden bundled approval patterns",
    /Forbidden Bundled Approval Patterns/i.test(doc)
  );
  ok(
    "doc pre-commit commit push deploy approval separation",
    /Pre-Commit \/ Commit \/ Push \/ Deploy Approval Separation/i.test(doc)
  );
  ok(
    "doc runtime change approval boundary",
    /Runtime Change Approval Boundary/i.test(doc)
  );
  ok(
    "doc deploy approval boundary",
    /Deploy Approval Boundary/i.test(doc)
  );
  ok(
    "doc production approval boundary",
    /Production Approval Boundary/i.test(doc)
  );
  ok(
    "doc secret env approval boundary",
    /Secret \/ Env Approval Boundary/i.test(doc)
  );
  ok(
    "doc gemini provider invocation approval boundary",
    /Gemini \/ Provider Invocation Approval Boundary/i.test(doc)
  );
  ok(
    "doc prompt provider payload approval boundary",
    /Prompt \/ Provider Payload Approval Boundary/i.test(doc)
  );
  ok(
    "doc user-visible behavior approval boundary",
    /User-Visible Behavior Approval Boundary/i.test(doc)
  );
  ok(
    "doc monitoring logging approval boundary",
    /Monitoring \/ Logging Approval Boundary/i.test(doc)
  );
  ok(
    "doc AI log persistence approval boundary",
    /AI Log Persistence Approval Boundary/i.test(doc)
  );
  ok(
    "doc pilot start approval boundary",
    /Pilot Start Approval Boundary/i.test(doc)
  );
  ok(
    "doc incident response approval boundary",
    /Incident Response Approval Boundary/i.test(doc)
  );
  ok(
    "doc rollback execution approval boundary",
    /Rollback Execution Approval Boundary/i.test(doc)
  );
  ok(
    "doc production promotion approval boundary",
    /Production Promotion Approval Boundary/i.test(doc)
  );
  ok(
    "doc evidence requirements per approval class",
    /Evidence Requirements per Approval Class/i.test(doc)
  );
  ok(
    "doc report requirements per approval class",
    /Report Requirements per Approval Class/i.test(doc)
  );
  ...
  ok(
    "doc approval separation table",
    /Approval Separation Table/i.test(doc)
  );
  ok("doc version boundary table", /Version Boundary Table/i.test(doc));
  ok(
    "doc future execution packet requirements",
    /Future Execution Packet Requirements/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc approval separation artifact only",
    /approval separation artifact|separation rules only/i.test(doc)
  );
}

// --- approval separation table rows ---
{
  ok("doc class Commit Approval", /Commit Approval/i.test(doc));
  ok("doc class Push Approval", /Push Approval/i.test(doc));
  ok("doc class Deploy Approval", /Deploy Approval/i.test(doc));
  ok("doc class Secret Env Approval", /Secret \/ Env Approval/i.test(doc));
  ok(
    "doc class Gemini Provider Invocation Approval",
    /Gemini \/ Provider Invocation Approval/i.test(doc)
  );
  ok(
    "doc class Prompt Provider Payload Approval",
    /Prompt \/ Provider Payload Approval/i.test(doc)
  );
  ok(
    "doc class Runtime UI Behavior Approval",
    /Runtime UI \/ Behavior Approval/i.test(doc)
  );
  ok(
    "doc class Runtime Redaction Integration Approval",
    /Runtime Redaction Integration Approval/i.test(doc)
  );
  ok(
    "doc class Monitoring Logging Approval",
    /Monitoring \/ Logging Approval/i.test(doc)
  );
  ok(
    "doc class AI Log Persistence Approval",
    /AI Log Persistence Approval/i.test(doc)
  );
  ok(
    "doc class Staging Pilot Start Approval",
    /Staging Pilot Start Approval/i.test(doc)
  );
  ok(
    "doc class Production Pilot Start Approval",
    /Production Pilot Start Approval/i.test(doc)
  );
  ok(
    "doc class Production Promotion Approval",
    /Production Promotion Approval/i.test(doc)
  );
  ok(
    "doc class Incident Response Execution Approval",
    /Incident Response Execution Approval/i.test(doc)
  );
  ok(
    "doc class Rollback Execution Approval",
    /Rollback Execution Approval/i.test(doc)
  );
  ok("doc references v6.5J", doc.includes("v6.5J"));
...
// --- version boundary table rows ---
{
  ok("doc version Docs-only packet", /Docs-only packet/i.test(doc));
  ok("doc version Static test packet", /Static test packet/i.test(doc));
  ok("doc version Runtime code change", /Runtime code change/i.test(doc));
  ok("doc version Secret wiring execution", /Secret wiring execution/i.test(doc));
  ok("doc version Real provider invocation", /Real provider invocation/i.test(doc));
  ok("doc version Staging deploy", /Staging deploy/i.test(doc));
  ok("doc version Production deploy", /Production deploy/i.test(doc));
  ok("doc version Staging pilot start", /Staging pilot start/i.test(doc));
  ok("doc version Production pilot start", /Production pilot start/i.test(doc));
  ok("doc version Rollback execution", /Rollback execution/i.test(doc));
}
...
// --- no secrets / PII / sensitive samples ---
{
  ...
  ok(
    "doc v65k does not authorize execution alone",
    /v6\.5K alone satisfies none|Execution Allowed by v6\.5K\?\s*=\s*No/i.test(
      doc
    )
  );
...
// --- package script ---
{
  ok(
    "package v65k script",
    pkg.includes("test:v65k-execution-approval-separation-version-boundary-packet")
  );
  ok(
...
console.log(
  "\nDone v6.5K Execution Approval Separation & Version Boundary Packet tests."
);

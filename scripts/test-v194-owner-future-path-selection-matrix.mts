/**
 * v19.4 owner future path selection matrix validator
 * Static checks only. Matrix-only, future-path-selection-matrix-only, review-only.
 *
 * npm run test:v19.4
 */
import { existsSync, readFileSync } from "node:fs";

const DOC_PATH = "docs/v19.4-owner-future-path-selection-matrix.md";
const FIXTURE_PATH = "docs/examples/v19.4-owner-future-path-selection-matrix.synthetic.json";
...
const requiredSections = [
  ...
  "currentStatusAfterV193Section",
  "closedBaselineCarryForwardSection",
  "v194ScopeSection",
  "matrixOnlyReminderSection",
  "futurePathSelectionMatrixOnlyReminderSection",
  ...
  "v19PathMatrixPurposeSection",
  "futurePathMatrixComparisonSection",
  "futurePathMatrixNotSelectionSection",
  "ownerDecisionNotIncludedSection",
  "noFuturePathSelectedSection",
  "noRequestTypeSelectedSection",
  "reviewOnlyPathOptionSection",
  "executionRequestDraftPathOptionSection",
  "realValueInsertionDraftPathOptionSection",
  "ownerApprovalPacketPathOptionSection",
  "controlledOneRunPrepPathOptionSection",
...
const matrixPathOptions = [
  "FUTURE_PATH_OPTION_REVIEW_ONLY_PLACEHOLDER",
  "FUTURE_PATH_OPTION_EXECUTION_REQUEST_DRAFT_PLACEHOLDER",
  "FUTURE_PATH_OPTION_REAL_VALUE_INSERTION_DRAFT_PLACEHOLDER",
  "FUTURE_PATH_OPTION_OWNER_APPROVAL_PACKET_PLACEHOLDER",
  "FUTURE_PATH_OPTION_CONTROLLED_ONE_RUN_PREP_PLACEHOLDER"
] as const;

console.log("=== v19.4 Owner Future Path Selection Matrix Validation ===\n");
...
ok(
  "doc/fixture identify v19.4 owner future path selection matrix correctly",
  /v19\.4 - Owner Future Path Selection Matrix/.test(doc) &&
    fixtureRaw.includes("\"version\": \"v19.4\"") &&
    fixtureRaw.includes("\"executionType\": \"owner-future-path-selection-matrix only\"")
);

ok(
  "v19.4 scope status flags appear",
...
if (packageParsed && typeof packageParsed === "object") {
  ...
  ok("package has test:v19.4 script", scripts["test:v19.4"] === "tsx scripts/test-v194-owner-future-path-selection-matrix.mts");
}
...
if (fixtureParsed && typeof fixtureParsed === "object") {
  ...
  ok(
    "baseline confirms v13-v19.3 carry-forward",
    baseline.v13 === "CLOSED" &&
      baseline.v14 === "CLOSED" &&
      baseline.v15 === "CLOSED" &&
  ...
  const options = asRecord(root.futurePathOptionPlaceholders);
  ok("future path matrix placeholders exist", matrixPathOptions.every((key) => typeof options[key] === "object"));
  ok(
    "no path option selected and placeholders only",
    matrixPathOptions.every((key) => {
      const item = asRecord(options[key]);
      return (
        item.placeholderPathOnly === true &&
        item.matrixReviewOnly === true &&
        item.selected === false &&
  ...
  const req = asRecord(root.matrixSeparationRequirements);
  ok(
    "matrix separation requirements exist",
...
ok(
  ...
  doc.includes("V19.4 OWNER FUTURE PATH SELECTION MATRIX CLOSED - READY FOR OWNER REVIEW ONLY / NO EXECUTION") &&
    fixtureRaw.includes("\"finalDecision\": \"V19.4 OWNER FUTURE PATH SELECTION MATRIX CLOSED - READY FOR OWNER REVIEW ONLY / NO EXECUTION\"")
);

console.log(`\nDone v19.4 validation - ${pass} PASS, ${fail} FAIL.\n`);

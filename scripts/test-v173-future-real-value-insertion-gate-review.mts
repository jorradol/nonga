/**
 * v17.3 future real-value insertion gate review validator
 * Static checks only. Gate-review-only and placeholder-only.
 *
 * npm run test:v17.3
 */
import { existsSync, readFileSync } from "node:fs";

const DOC_PATH = "docs/v17.3-future-real-value-insertion-gate-review.md";
const FIXTURE_PATH = "docs/examples/v17.3-future-real-value-insertion-gate-review.synthetic.json";
...
console.log("=== v17.3 Future Real-Value Insertion Gate Review Validation ===\n");
...
ok(
  "doc/fixture identify v17.3 correctly",
  /v17\.3 — Future Real-Value Insertion Gate Review/.test(doc) &&
    fixtureRaw.includes("\"version\": \"v17.3\"") &&
    fixtureRaw.includes("\"executionType\": \"future-real-value-insertion-gate-review only\"")
);

ok(
  "status is gate-review-only no-execution placeholder-only not-owner-approval and not-real-value-insertion",
  hasEveryLine(doc, [
...
if (packageParsed && typeof packageParsed === "object") {
  ...
  ok(
    "package has test:v17.3 script",
    scripts["test:v17.3"] === "tsx scripts/test-v173-future-real-value-insertion-gate-review.mts"
  );
}
...
if (fixtureParsed && typeof fixtureParsed === "object") {
  ...
  ok(
    ...
    baseline["v13"] === "CLOSED" &&
      ...
      baseline["v17.3"] === "FUTURE REAL-VALUE INSERTION GATE REVIEW ONLY / NO EXECUTION"
  );

  const gates = asRecord(root.realValueInsertionGateCategories);
  ok(
    "real-value insertion gate categories complete",
    gates.ownerIntentGate === "OWNER_INTENT_PLACEHOLDER_ONLY" &&
      gates.ownerReviewGate === "OWNER_REVIEW_PLACEHOLDER_ONLY" &&
      gates.ownerApprovalGate === "OWNER_APPROVAL_PLACEHOLDER_ONLY" &&
      gates.exactCommandGate === "EXACT_COMMAND_PLACEHOLDER_ONLY" &&
      gates.exactTargetGate === "EXACT_TARGET_PLACEHOLDER_ONLY" &&
      gates.exactExpectedEvidenceGate === "EXACT_EXPECTED_EVIDENCE_PLACEHOLDER_ONLY" &&
      gates.repoHeadGate === "REPO_HEAD_CHECK_PLACEHOLDER_ONLY" &&
      gates.originHeadGate === "ORIGIN_HEAD_CHECK_PLACEHOLDER_ONLY" &&
      gates.workingTreeGate === "WORKING_TREE_CHECK_PLACEHOLDER_ONLY" &&
      gates.requiredTestsGate === "REQUIRED_TESTS_PLACEHOLDER_ONLY" &&
      gates.oneRunOnlyGate === "ONE_RUN_ONLY_PLACEHOLDER_ONLY" &&
      gates.noAutomaticRetryGate === "NO_AUTOMATIC_RETRY_PLACEHOLDER_ONLY" &&
      gates.noSecondRunGate === "NO_SECOND_RUN_PLACEHOLDER_ONLY" &&
      gates.rollbackOwnerGate === "ROLLBACK_OWNER_PLACEHOLDER_ONLY" &&
      gates.killSwitchOwnerGate === "KILL_SWITCH_OWNER_PLACEHOLDER_ONLY" &&
      gates.stopConditionsGate === "STOP_CONDITIONS_PLACEHOLDER_ONLY" &&
      gates.sanitizedEvidenceGate === "SANITIZED_EVIDENCE_BUNDLE_PLACEHOLDER_ONLY" &&
      gates.tokenSecretPiiGate === "TOKEN_SECRET_PII_CHECK_PLACEHOLDER_ONLY" &&
      gates.publicProductionGate === "PUBLIC_PRODUCTION_CHECK_PLACEHOLDER_ONLY" &&
      gates.realLeadGate === "REAL_LEAD_CHECK_PLACEHOLDER_ONLY" &&
      gates.realDealerGate === "REAL_DEALER_CHECK_PLACEHOLDER_ONLY" &&
      gates.runtimeProviderGeminiGate === "RUNTIME_PROVIDER_GEMINI_CHECK_PLACEHOLDER_ONLY" &&
      gates.liveEndpointManualGuessGate === "LIVE_ENDPOINT_MANUAL_GUESS_CHECK_PLACEHOLDER_ONLY" &&
      gates.thorDealerImportGate === "THOR_DEALER_IMPORT_CHECK_PLACEHOLDER_ONLY"
  );

  ok(
    "required gate statuses complete",
    Array.isArray(root.requiredGateStatuses) &&
      root.requiredGateStatuses.includes("OWNER_INTENT_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("OWNER_REVIEW_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("OWNER_APPROVAL_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("EXACT_COMMAND_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("EXACT_TARGET_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("EXACT_EXPECTED_EVIDENCE_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("REPO_HEAD_CHECK_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("ORIGIN_HEAD_CHECK_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("WORKING_TREE_CHECK_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("REQUIRED_TESTS_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("ONE_RUN_ONLY_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("NO_AUTOMATIC_RETRY_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("NO_SECOND_RUN_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("ROLLBACK_OWNER_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("KILL_SWITCH_OWNER_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("STOP_CONDITIONS_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("SANITIZED_EVIDENCE_BUNDLE_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("TOKEN_SECRET_PII_CHECK_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("PUBLIC_PRODUCTION_CHECK_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("REAL_LEAD_CHECK_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("REAL_DEALER_CHECK_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("RUNTIME_PROVIDER_GEMINI_CHECK_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("LIVE_ENDPOINT_MANUAL_GUESS_CHECK_PLACEHOLDER_ONLY") &&
      root.requiredGateStatuses.includes("THOR_DEALER_IMPORT_CHECK_PLACEHOLDER_ONLY")
  );

  ok(
    "what v17.3 does not approve list complete",
    Array.isArray(root.whatV173DoesNotApprove) && root.whatV173DoesNotApprove.length >= 19
  );

  const freshApproval = asRecord(root.freshOwnerApprovalSeparation);
  ok(
    "fresh owner approval separation complete and true",
    Object.keys(freshApproval).length >= 8 &&
  ...
  ok(
    ...
    Object.keys(guard).length >= 21 && Object.values(guard).every((value) => value === false)
  );
  ...
  ok(
    ...
    Array.isArray(root.safeNegativeProhibitionContext) &&
      ...
      root.safeNegativeProhibitionContext.includes("this gate review does not authorize execution")
  );

  ok(
    "owner-friendly thai summary includes required lines",
    Array.isArray(root.ownerFriendlyThaiSummary) &&
      root.ownerFriendlyThaiSummary.includes("v17.3 เป็นด่านตรวจ ก่อนอนาคตจะพิจารณาใส่ค่าจริง") &&
  ...
  ok(
    "gate review outcome options complete and non-authorizing",
    Array.isArray(root.gateReviewOutcomeOptions) &&
      root.gateReviewOutcomeOptions.includes(
        "GATE REVIEW COMPLETE — READY TO CONSIDER FUTURE REAL-VALUE INSERTION REQUEST ONLY"
      ) &&
      root.gateReviewOutcomeOptions.includes("HOLD GATE REVIEW — CLARIFICATION NEEDED") &&
      root.gateReviewOutcomeOptions.includes("REVISE GATE REVIEW — DOCS/TESTS/FIXTURES ONLY") &&
      root.gateReviewOutcomeOptions.includes("STOP BEFORE REAL-VALUE INSERTION — NO-GO GATE OUTCOME") &&
      root.gateOutcomeOptionsDoNotAuthorizeRealValueInsertionExecutionOrGo === true
  );
  ...
  ok(
    "final decision is gate review closure only",
    root.finalDecision ===
      "V17.3 GATE REVIEW CLOSED — READY TO CONSIDER FUTURE REAL-VALUE INSERTION REQUEST ONLY / NO EXECUTION"
  );
}

ok(
  "doc includes all real-value insertion gate category names",
  hasEveryLine(doc, [
    "ownerIntentGate",
    "ownerReviewGate",
    "ownerApprovalGate",
    "exactCommandGate",
    "exactTargetGate",
    "exactExpectedEvidenceGate",
    "repoHeadGate",
    "originHeadGate",
    "workingTreeGate",
    "requiredTestsGate",
    "oneRunOnlyGate",
    "noAutomaticRetryGate",
    "noSecondRunGate",
    "rollbackOwnerGate",
    "killSwitchOwnerGate",
    "stopConditionsGate",
    "sanitizedEvidenceGate",
    "tokenSecretPiiGate",
    "publicProductionGate",
    "realLeadGate",
    "realDealerGate",
    "runtimeProviderGeminiGate",
    "liveEndpointManualGuessGate",
    "thorDealerImportGate"
  ])
);

ok(
  "doc has required gate status placeholder strings",
  hasEveryLine(doc, [
...
ok(
  "doc includes what v17.3 does not approve list",
  hasEveryLine(doc, [
    "GO",
    "execution",
...
ok(
  "doc includes gate review outcome options and non-authorizing statement",
  hasEveryLine(doc, [
    "GATE REVIEW COMPLETE — READY TO CONSIDER FUTURE REAL-VALUE INSERTION REQUEST ONLY",
...
console.log(`\nDone v17.3 validation - ${pass} PASS, ${fail} FAIL.\n`);

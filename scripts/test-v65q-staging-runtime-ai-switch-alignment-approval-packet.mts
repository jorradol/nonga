/**
 * v6.5Q — Staging Runtime AI Switch Alignment Approval Packet (static validation only)
...
const AUTO_EXEC_AUTH_PATTERNS = [
  ...
  /v6\.5Q\s+approves\s+production\s+pilot\s+automatically/i,
  /v6\.5Q\s+authorizes\s+production\s+pilot\s+start/i,
  /v6\.5Q\s+captures\s+approval\s+in\s+v6\.5Q\s+slice/i,
  /v6\.5Q\s+authorizes\s+runtime\s+switch\s+alignment\s+execution/i,
  ...
  /runtime\s+switch\s+alignment\s+is\s+authorized\s+by\s+v6\.5Q\s+alone/i,
  /v6\.5Q\s+approved\b/i,
];

const BAD_APPROVED_PATTERNS = [
  /Approval Captured\?\s*=\s*Yes/i,
  /Execution Allowed\?\s*=\s*Yes/i,
  /May Execute from v6\.5Q\?\s*=\s*Yes/i,
  /Runtime Switch Alignment Approved\?\s*=\s*Yes/i,
  /Runtime Switch Alignment Executed\?\s*=\s*Yes/i,
  ...
  /runtime\s+switch\s+alignment\s+approved\s+for/i,
  /all\s+gates\s+executed/i,
  /execution\s+authorized\s+by\s+v6\.5Q/i,
  /runtime\s+switch\s+alignment\s+is\s+approved/i,
  /staging\s+runtime\s+switch\s+alignment\s+approved/i,
];

const HEAD_SHA = "f93be526c749f56b0168f3ee655a00079551b5b1";
...
const selfSrc = readFileSync(
  "scripts/test-v65q-staging-runtime-ai-switch-alignment-approval-packet.mts",
  "utf8"
);
...
{
  ok("v6.5Q doc exists", doc.length > 15000);
  ok("doc v6.5Q label", doc.includes("v6.5Q"));
...
// --- safety positioning ---
{
  ok("doc HEAD f93be52", doc.includes(HEAD_SHA) || doc.includes("f93be52"));
  ...
  ok("doc no secret creation", /does not create|no secret creation/i.test(doc));
  ok("doc no secret read", /does not read secrets|no secret read/i.test(doc));
  ok("doc no secret binding", /does not bind|no secret binding/i.test(doc));
  ok(
    "doc no env update",
  ...
  ok(
    ...
    /does not execute Cloud Run CLI|Cloud Run CLI.*Prohibited|Cloud Run CLI Executed\?\s*=\s*No/i.test(
      doc
    )
  );
  ok(
  ...
  ok(
    "doc no runtime switch alignment approval captured",
    /no runtime switch alignment approval captured|No runtime switch alignment approval is captured/i.test(
      doc
    )
  );
  ok(
  ...
  ok(
    "doc may execute from v65q is no",
    /May Execute from v6\.5Q\?\s*=\s*No|May Execute from v6\.5Q\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
    "doc runtime switch alignment approved is no",
    /Runtime Switch Alignment Approved\?\s*=\s*No|Runtime Switch Alignment Approved\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
    "doc runtime switch alignment executed is no",
    /Runtime Switch Alignment Executed\?\s*=\s*No|Runtime Switch Alignment Executed\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
  ...
  ok(
    ...
    /no gate executed|not executed|Does not state any gate executed/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc does not state runtime switch alignment approved",
    /Does not state runtime switch alignment approved|does not state runtime switch alignment approved/i.test(
      doc
    )
  );
  ok(
...
// --- required sections ---
{
  ...
  ok(
    "doc not authorization execute deploy activate production pilot runtime switch alignment",
    /not authorization to execute\/deploy\/activate\/production\/pilot\/runtime switch alignment|not approval to deploy/i.test(
      doc
    )
  );
  ok("doc current state summary", /Current State Summary/i.test(doc));
  ok(
    "doc Gate A execution carry-forward",
    /Gate A Execution Carry-Forward/i.test(doc)
  );
  ok(
    "doc Gate A warnings carry-forward",
    /Gate A Warnings Carry-Forward/i.test(doc)
  );
  ok(
    "doc what v65q is is not",
    /What v6\.5Q Is \/ Is Not/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc relationship to v65p final gono-go",
    /Relationship to v6\.5P Final Go\/No-Go/i.test(doc)
  );
  ok(
    "doc why v65q is needed before Gate B",
    /Why v6\.5Q Is Needed Before Gate B/i.test(doc)
  );
  ok(
    "doc observed staging runtime switch state",
    /Observed Staging Runtime Switch State/i.test(doc)
  );
  ok(
    "doc risk analysis of current switch state",
    /Risk Analysis of Current Switch State/i.test(doc)
  );
  ok("doc user-visible AI risk", /User-Visible AI Risk/i.test(doc));
  ok(
    "doc admin shadow real provider risk",
    /Admin Shadow Real Provider Risk/i.test(doc)
  );
  ok("doc Gemini cost risk", /Gemini Cost Risk/i.test(doc));
  ok("doc production exclusion", /Production Exclusion/i.test(doc));
  ok("doc deploy exclusion", /Deploy Exclusion/i.test(doc));
  ok(
    "doc Gemini invocation exclusion",
    /Gemini Invocation Exclusion/i.test(doc)
  );
  ok("doc pilot start exclusion", /Pilot Start Exclusion/i.test(doc));
  ok(
    "doc recommended future switch alignment",
    /Recommended Future Switch Alignment/i.test(doc)
  );
  ok("doc future execution scope", /Future Execution Scope/i.test(doc));
  ok("doc future execution non-scope", /Future Execution Non-Scope/i.test(doc));
  ok(
  ...
  ok(
    "doc owner checklist before approval",
    /Owner Checklist Before Approval/i.test(doc)
  );
  ok(
    "doc technical checklist before approval",
    /Technical Checklist Before Approval/i.test(doc)
  );
  ok(
    "doc safety checklist before approval",
    /Safety Checklist Before Approval/i.test(doc)
  );
  ok(
    "doc privacy checklist before approval",
    /Privacy Checklist Before Approval/i.test(doc)
  );
  ok(
    "doc cost checklist before approval",
    /Cost Checklist Before Approval/i.test(doc)
  );
  ok(
    "doc environment handling checklist",
    /Environment Handling Checklist/i.test(doc)
  );
  ok("doc command boundary", /Command Boundary/i.test(doc));
  ok(
    "doc commands prohibited in v65q",
    /Commands Prohibited in v6\.5Q/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc Future Gate B prerequisite status",
    /Future Gate B Prerequisite Status/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc observed W-01 user visible true",
    /NONGA_AI_USER_VISIBLE_ENABLED.*`true`|W-01.*true/i.test(doc)
  );
  ok(
    "doc observed W-02 admin shadow true",
    /NONGA_AI_ADMIN_SHADOW_REAL_PROVIDER_ENABLED.*`true`|W-02.*true/i.test(doc)
  );
  ok(
    "doc DEFAULT_AI_PROVIDER_STATUS",
    /DEFAULT_AI_PROVIDER_STATUS/i.test(doc)
  );
  ok(
    "doc v65q exec future execution",
    /v6\.5Q\.EXEC/i.test(doc)
  );
  ok(
    "doc approval wording v65q exec",
    /execute v6\.5Q\.EXEC.*Staging Runtime AI Switch Alignment/i.test(doc)
  );
  ok(
    "doc pre-existing runtime state",
    /pre-existing runtime state|pre-existing/i.test(doc)
  );
  ok(
    "doc not caused by Gate A",
    /not caused by Gate A|Not caused by Gate A/i.test(doc)
  );
  ok(
    "doc not fixed in v6.5Q",
    /not fixed in v6\.5Q|Not fixed in v6\.5Q/i.test(doc)
  );
}
...
// --- no secrets / PII / sensitive samples ---
{
  ...
  ok(
    "doc v65q does not authorize execution alone",
    /May Execute from v6\.5Q\?\s*=\s*No|Execution Allowed\?\s*=\s*No/i.test(doc)
  );
...
// --- package script ---
{
  ok(
    "package v65q script",
    pkg.includes(
      "test:v65q-staging-runtime-ai-switch-alignment-approval-packet"
    )
  );
  ok(
...
console.log(
  "\nDone v6.5Q Staging Runtime AI Switch Alignment Approval Packet tests."
);

 ...
 * npm run test:v70l-authenticated-dealer-smoke-owner-decision-closure
 *
...
const DOC_PATH =
  "docs/v7.0L-authenticated-dealer-smoke-owner-decision-closure.md";
...
ok("phase name v7.0L", /v7\.0L/i.test(doc));
ok(
  "phase title owner decision closure",
  /Owner Decision Closure for Authenticated Dealer Smoke Execution/i.test(doc)
);
ok(
  "branch feature/chat-image-attachment-v1",
  doc.includes("feature/chat-image-attachment-v1")
);
ok("baseline 8926221", doc.includes("8926221"));

// --- verdict ---
ok(
  "verdict OWNER DECISION CLOSED (PREP APPROVED)",
  /PARTIAL — OWNER DECISION CLOSED \(AUTHENTICATED SMOKE EXECUTION PREP APPROVED\)|PARTIAL -- OWNER DECISION CLOSED \(AUTHENTICATED SMOKE EXECUTION PREP APPROVED\)/i.test(
    doc
  )
);
...
ok(
  "why owner decision closure section",
  /Why owner decision closure is needed/i.test(doc)
);
ok("preconditions section", /Preconditions from v7\.0A\.\.v7\.0K/i.test(doc));
ok(
  "owner decision closure section",
  /Owner decision closure \(OD-K1\.\.OD-K9\)/i.test(doc)
);
ok("scope of OD-K1 yes section", /Scope of OD-K1 = YES/i.test(doc));
ok(
...
ok("what this closure allows section", /What this closure allows/i.test(doc));
ok(
  "what this closure does not allow section",
  /What this closure does not allow/i.test(doc)
);
ok("relationship to v7.0K section", /Relationship to v7\.0K/i.test(doc));
ok(
...
// --- OD-K1 = YES ---
ok("decision item OD-K1", doc.includes("OD-K1"));
ok(
  "OD-K1 = YES",
  /\| OD-K1 \|[^\n]*\| \*\*YES\*\* \|/.test(doc)
);
...
ok(
  ...
  /OD-K1 = \*\*YES[^\n]*OD-K2\.\.OD-K9 = \*\*PASS/i.test(doc)
);

// --- OD-K1 = YES means prep/execution slice only, not revenue GO ---
ok(
  "od-k1 yes means prep only not revenue go",
  /OD-K1 = YES หมายถึงอนุญาตให้.*เตรียม\/ทำ slice execution ถัดไป.*เท่านั้น.*ไม่ใช่.*revenue pilot GO/i.test(
    doc
  )
);

// --- guardrail mapping checks (OD-K6/K7/K8 keep blocks) ---
ok("od-k6 thor blocked", /OD-K6[^\n]*Thor runtime import[^\n]*BLOCKED/i.test(doc));
ok(
  "od-k7 gemini blocked",
  /OD-K7[^\n]*user-visible Gemini[^\n]*BLOCKED/i.test(doc)
);
ok(
  "od-k8 deploy/signup blocked",
  /OD-K8[^\n]*production deploy \/ public signup[^\n]*BLOCKED/i.test(doc)
);
ok(
  "od-k9 abort on pii/leak",
  /OD-K9[^\n]*PII \/ lead leak \/ cross-dealer leak[^\n]*abort/i.test(doc)
);
...
// --- v7.0L is NOT authenticated smoke execution / not smoke result ---
ok(
  "not authenticated smoke execution",
  /ไม่ใช่.*authenticated smoke execution|v7\.0L \*\*ไม่ใช่\*\* authenticated smoke execution/i.test(
    doc
  )
);
ok(
  "not smoke result",
  /ไม่ใช่.*smoke result|v7\.0L \*\*ไม่ใช่\*\*.*smoke result/i.test(doc)
);
...
// --- no overclaim smoke PASS rule ---
ok(
  "no overclaim smoke pass rule",
  /ห้าม overclaim ว่า authenticated smoke PASS/i.test(doc)
);
...
ok(
  "actual execution in separate slice",
  /execution slice ภายหลัง|slice แยกหลัง owner approve|slice execution ถัดไป/i.test(
    doc
  )
);
...
// --- closure not pilot/production/revenue go ---
ok(
  "closure not pilot/production/revenue go",
  /closure \*\*ยังไม่ใช่\*\* pilot-ready \/ production-ready \/ revenue GO|ยังไม่ใช่ pilot-ready/i.test(
    doc
  )
);
...
ok(
  ...
  pkg.includes(
    "test:v70l-authenticated-dealer-smoke-owner-decision-closure"
  )
);

console.log(
  "\nDone v7.0L owner decision closure for authenticated dealer smoke execution tests.\n"
);

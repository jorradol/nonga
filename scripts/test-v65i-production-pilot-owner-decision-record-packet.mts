/**
 * v6.5I — Production Pilot Owner Decision Record Packet (static validation only)
...
const AUTO_EXEC_AUTH_PATTERNS = [
  ...
  /v6\.5I\s+approves\s+production\s+pilot\s+automatically/i,
  /v6\.5I\s+authorizes\s+production\s+pilot\s+start/i,
  /v6\.5I\s+captures\s+owner\s+decision\s+in\s+v6\.5I\s+slice/i,
  /owner\s+has\s+decided\s+Go\s+for\s+production/i,
  /packet\s+authorizes\s+production\s+promotion\s+automatically/i,
  /v6\.5I\s+authorizes\s+secret\s+wiring\s+automatically/i,
  /production\s+pilot\s+is\s+approved/i,
];

const BAD_APPROVED_PATTERNS = [
  /Decision Captured by v6\.5I\?\s*=\s*Yes/i,
  /Approval Captured by v6\.5I\?\s*=\s*Yes/i,
  /Execution Allowed by v6\.5I\?\s*=\s*Yes/i,
  /gate\s+executed\s+for\s+production/i,
  /all\s+gates\s+executed/i,
  /execution\s+authorized\s+by\s+v6\.5I/i,
];

const HEAD_SHA = "8ca2d56bc34458de609d0017b3f98baccb95ecfa";
...
const selfSrc = readFileSync(
  "scripts/test-v65i-production-pilot-owner-decision-record-packet.mts",
  "utf8"
);
...
{
  ok("v6.5I doc exists", doc.length > 12000);
  ok("doc v6.5I label", doc.includes("v6.5I"));
...
// --- safety positioning ---
{
  ok("doc HEAD 8ca2d56", doc.includes(HEAD_SHA) || doc.includes("8ca2d56"));
  ...
  ok(
    "doc no owner decision captured",
    /No owner decision is captured|no owner decision captured/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc owner decision record packet only",
    /owner decision record packet|Production pilot owner decision record packet/i.test(
      doc
    )
  );
  ok(
    "doc decision captured by v65i is no",
    /Decision Captured by v6\.5I\?\s*=\s*No|Decision Captured by v6\.5I\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
    "doc approval captured by v65i is no",
    /Approval Captured by v6\.5I\?\s*=\s*No|Approval Captured by v6\.5I\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
    "doc execution allowed by v65i is no",
    /Execution Allowed by v6\.5I\?\s*=\s*No|Execution Allowed by v6\.5I\?\s*\|\s*\*\*No\*\*/i.test(
      doc
    )
  );
  ok(
  ...
  ok(
    "doc does not state owner decided Go",
    /Does not state owner decided Go|does not state owner decided Go/i.test(doc)
  );
}
...
// --- required sections ---
{
  ...
  ok(
    "doc what v65i is is not",
    /What v6\.5I Is \/ Is Not/i.test(doc)
  );
  ok(
    "doc owner decision record definition",
    /Owner Decision Record Definition/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc owner decision record form template",
    /Owner Decision Record Form \/ Template/i.test(doc)
  );
  ok(
    "doc required evidence inventory",
    /Required Evidence Inventory/i.test(doc)
  );
  ok(
    "doc required approvals inventory",
    /Required Approvals Inventory/i.test(doc)
  );
  ok("doc decision preconditions", /Decision Preconditions/i.test(doc));
  ok("doc Go decision requirements", /Go Decision Requirements/i.test(doc));
  ok(
    "doc Defer decision requirements",
    /Defer Decision Requirements/i.test(doc)
  );
  ok(
    "doc No-Go decision requirements",
    /No-Go Decision Requirements/i.test(doc)
  );
  ...
  ok(
    "doc staging prerequisite status",
    /Staging Prerequisite Status/i.test(doc)
  );
  ok(
    "doc production pilot approval status",
    /Production Pilot Approval Status/i.test(doc)
  );
  ok(
    "doc privacy data handling decision dependency",
    /Privacy \/ Data Handling Decision Dependency/i.test(doc)
  );
  ok(
    "doc cost guard quota decision dependency",
    /Cost Guard \/ Quota Decision Dependency/i.test(doc)
  );
  ok(
    "doc incident rollback decision dependency",
    /Incident \/ Rollback Decision Dependency/i.test(doc)
  );
  ok(
    "doc user-visible behavior decision dependency",
    /User-Visible AI Behavior Decision Dependency/i.test(doc)
  );
  ok(
    "doc monitoring kill switch decision dependency",
    /Monitoring \/ Kill Switch Decision Dependency/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc post-decision review requirements",
    /Post-Decision Review Requirements/i.test(doc)
  );
  ok(
    "doc pilot execution handoff requirements",
    /Pilot Execution Handoff Requirements/i.test(doc)
  );
  ok(
    "doc decision record limitations",
    /Decision Record Limitations/i.test(doc)
  );
  ok(
  ...
  ok(
    "doc owner decision record table",
    /Owner Decision Record Table/i.test(doc)
  );
  ok(
    "doc decision template only not captured",
    /template only|not captured decision|decision record template only/i.test(doc)
  );
}

// --- owner decision record table rows ---
{
  ok(
...
// --- no secrets / PII / sensitive samples ---
{
  ...
  ok(
    "doc v65i does not authorize execution alone",
    /v6\.5I alone satisfies none|Execution Allowed by v6\.5I\?\s*=\s*No/i.test(
      doc
    )
  );
...
// --- package script ---
{
  ok(
    "package v65i script",
    pkg.includes("test:v65i-production-pilot-owner-decision-record-packet")
  );
  ok(
...
console.log(
  "\nDone v6.5I Production Pilot Owner Decision Record Packet tests."
);

/**
 * v13.15J owner credential action guide validator
 * Guide/readiness-only checks. No provider call and no secret exposure.
 *
 * npm run test:v13.15J
 */
...
const DOC_PATH = "docs/v13.15J-owner-credential-action-guide.md";
const SELF_PATH = "scripts/test-v1315j-owner-credential-action-guide.mts";

const SYNTHETIC_UID = "synthetic-owner-v1315j";
...
console.log("=== v13.15J Owner Credential Action Guide Validation ===\n");
...
ok("doc has substantial content", doc.length > 4000, `${doc.length} chars`);
ok("doc includes firebase auth instruction", /Firebase authenticated owner\/operator path instruction/i.test(doc));
ok("doc includes secret handling rules", /Secret handling rule/i.test(doc));
ok("doc includes boundary check plan", /Authenticated boundary check plan/i.test(doc));
ok("doc includes runtime key presence plan", /GEMINI_API_KEY.*presence confirmation plan/i.test(doc));
...
setUserVisibleGeminiCallerForTests(async () => {
  geminiCallCount += 1;
  throw new Error("Gemini must not be called in v13.15J guide phase");
});
...
ok("one-run remains unconsumed marker in doc", /no one-run consumption: yes/i.test(doc));
...
console.log(`\nDone v13.15J validation - ${pass} PASS, ${fail} FAIL.\n`);

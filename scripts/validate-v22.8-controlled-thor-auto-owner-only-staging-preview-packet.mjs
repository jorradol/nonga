...
const DOC_PATH = "docs/v22.8-controlled-thor-auto-owner-only-staging-preview-packet.md";
const FIXTURE_PATH = "docs/examples/v22.8-controlled-thor-auto-owner-only-staging-preview-packet.example.json";
...
console.log("=== v22.8 controlled thor auto owner-only staging preview packet validator ===\n");
...
check("v22.7 baseline PASS", /PASS - v22\.7 controlled Thor Auto sample validation and staging preview prep prepared/i.test(doc));
check("v22.7 sample validation and staging preview prep only", /v22\.7 was sample validation and staging preview prep only/i.test(doc));
check("v22.7 forbidden fields found no", /v22\.7 found no forbidden fields/i.test(doc));
check("v22.7 no real market import", /v22\.7 did not import real market inventory/i.test(doc));
check("v22.7 no public preview published", /v22\.7 did not publish public preview/i.test(doc));
check("v22.7 no dealer-facing send", /v22\.7 did not perform dealer-facing send/i.test(doc));
check("v22.7 no real dealer contact action", /v22\.7 did not perform real dealer contact\/action/i.test(doc));
check("v22.7 no real lead customer data", /v22\.7 did not process real customer data/i.test(doc));
check("v22.7 no pii phone plate vin full registration data", /v22\.7 had no PII\/phone\/plate\/VIN\/full registration data/i.test(doc));
check("v22.7 no token header cookie secret env", /v22\.7 had no token\/header\/cookie\/secret\/env handling/i.test(doc));

check("owner-only staging preview packet only", /owner_only_staging_preview_packet_only/i.test(doc));
check("owner-browser view only", /owner_browser_view_only/i.test(doc));
check("no owner run in v22.8", /no owner run in v22\.8/i.test(doc));
...
check("Step 3 owner_only_staging_preview_packet_only", /Step 3: `owner_only_staging_preview_packet_only`/i.test(doc));
...
check(
  "owner-friendly owner-only staging preview explanation",
  /Owner-friendly explanation: owner-only staging preview packet means what/i.test(doc)
);
check("owner-browser-only preview checklist", /Owner-browser-only preview checklist/i.test(doc));
...
check("owner-browser login/session main path requirement", /use owner-browser login\/session as the main path later/i.test(doc));
...
check("future v22.9 approval phrase exact", doc.includes(REQUIRED_V229_APPROVAL_PHRASE));
check("future v22.9 documented only", /v22\.9 phrase is documented only/i.test(doc));

check("no approval to deploy", /no approval to deploy/i.test(doc));
check("no approval to run owner preview in v22.8", /no approval to run owner preview in v22\.8/i.test(doc));
...
check("exact v22.8 owner phrase present", doc.includes(REQUIRED_V228_APPROVAL_PHRASE));

check("fixture label indicates v22.8 packet only", /v22\.8 owner-only staging preview packet only/i.test(String(fixture.label)));
check("fixture baseline v22.7 pass true", fixture.baseline_v227_pass === true);
check(
  "fixture baseline v22.7 sample validation and staging preview prep only true",
  fixture.baseline_v227_sample_validation_and_staging_preview_prep_only === true
);
check("fixture baseline v22.7 forbidden fields found false", fixture.baseline_v227_forbidden_fields_found === false);
check("fixture owner_only_staging_preview_packet_only true", fixture.owner_only_staging_preview_packet_only === true);
check("fixture owner_browser_view_only true", fixture.owner_browser_view_only === true);
check("fixture no_owner_run_in_v22_8 true", fixture.no_owner_run_in_v22_8 === true);
...
check(
  "fixture step_3_status owner_only_staging_preview_packet_only",
  fixture.step_3_status === "owner_only_staging_preview_packet_only"
);
...
check(
  "fixture owner-friendly preview checklist array",
  Array.isArray(fixture.owner_friendly_preview_checklist) && fixture.owner_friendly_preview_checklist.length >= 5
);
...
check("future v22.9 phrase documented only true", fixture.future_v229_approval_phrase_documented_only === true);

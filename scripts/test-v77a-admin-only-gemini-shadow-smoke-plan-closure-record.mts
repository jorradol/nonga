/**
 * v7.7A — Admin-only Gemini Shadow Smoke Plan Closure Record (static validation only)
 * npm run test:v77a-admin-only-gemini-shadow-smoke-plan-closure-record
 *
...
const CLOSURE_DOC =
  "docs/v7.7A-admin-only-gemini-shadow-smoke-plan-closure-record.md";
const V77_DOC = "docs/v7.7-admin-only-gemini-shadow-smoke-plan.md";
const SELF_PATH =
  "scripts/test-v77a-admin-only-gemini-shadow-smoke-plan-closure-record.mts";
const PKG_PATH = "package.json";
const NPM_SCRIPT = "test:v77a-admin-only-gemini-shadow-smoke-plan-closure-record";

const HEAD_FULL = "b653f44c968cf4e73a6123a8e73eacea57bb6e26";
const HEAD_SHORT = "b653f44";
const PUSH_RANGE = "6844807..b653f44";
...
console.log("=== v7.7A Admin-only Gemini Shadow Smoke Plan Closure Record ===\n");
...
// --- doc exists + closure-only labelling ---
{
  ...
  ok("doc v7.7A label", doc.includes("v7.7A"));
  ...
  ok("doc references v7.7 source plan", doc.includes(V77_DOC) || doc.includes("v7.7"));
}

// --- baseline commit + push ---
{
  ok("head commit short b653f44", doc.includes(HEAD_SHORT));
  ok("head commit full sha", doc.includes(HEAD_FULL));
  ok("push range 6844807..b653f44", doc.includes(PUSH_RANGE));
  ...
  ok("prior baseline 6844807 stated", doc.includes("6844807"));
}

// --- plan/readiness/test design only, not runtime activation ---
{
  ok(
    "plan/readiness/test design only",
    /plan ?\/ ?readiness ?\/ ?test design only|plan\/readiness\/test design only/i.test(doc)
  );
...
// --- lead sending / consent / preview / final confirmation / dealer isolation NOT touched ---
{
...
// --- v7.7 plan summary + known blockers + next step + rollback + checklist ---
{
  ok("v7.7 plan summary present", /สาระสำคัญของ v7\.7 plan/i.test(doc));
  ok("deterministic authority map mentioned", /deterministic authority map/i.test(doc));
  ok("forbidden input fields mentioned", /forbidden input fields/i.test(doc));
  ok("fallback policy mentioned", /fallback (policy|deterministic)/i.test(doc));
  ok("kill switch drill mentioned", /kill switch drill/i.test(doc));
  ok("go/no-go criteria mentioned", /go\/no-go criteria|go\/no-go/i.test(doc));
  ...
  ok("closure verdict present", /closure verdict|v7\.7 CLOSED/i.test(doc));
}
...
// --- package.json npm script ---
{
  ...
  ok(
    ...
    pkg.includes("scripts/test-v77a-admin-only-gemini-shadow-smoke-plan-closure-record.mts")
  );
}

console.log(`\nDone v7.7A closure record validation — ${pass} PASS, ${fail} FAIL.\n`);

/**
 * v19.6 owner decision packet safety review gate validator
 * Static checks only. Safety-gate-only, decision-packet-safety-review-only, review-only.
 *
 * npm run test:v19.6
 */
import { existsSync, readFileSync } from "node:fs";

const DOC_PATH = "docs/v19.6-owner-decision-packet-safety-review-gate.md";
const FIXTURE_PATH = "docs/examples/v19.6-owner-decision-packet-safety-review-gate.synthetic.json";
...
const requiredSections = [
  ...
  "currentStatusAfterV195Section",
  "closedBaselineCarryForwardSection",
  "v196ScopeSection",
  "safetyGateOnlyReminderSection",
  "decisionPacketSafetyReviewOnlyReminderSection",
  ...
  "v195DecisionPacketDraftCarryForwardSection",
  "v19SafetyGatePurposeSection",
  "decisionPacketMisreadRiskReviewSection",
  "decisionPacketNotDecisionReviewSection",
  "decisionPacketNotApprovalReviewSection",
  "decisionPacketNotGoReviewSection",
  "decisionPacketNotExecutionReviewSection",
  ...
  "reviewOnlySafetyGateCheckSection",
  "executionRequestDraftSafetyGateCheckSection",
  "realValueInsertionDraftSafetyGateCheckSection",
  "ownerApprovalPacketSafetyGateCheckSection",
  "controlledOneRunPrepSafetyGateCheckSection",
...
const safetyGateChecks = [
  "FUTURE_SAFETY_GATE_CHECK_REVIEW_ONLY_PLACEHOLDER",
  "FUTURE_SAFETY_GATE_CHECK_EXECUTION_REQUEST_DRAFT_PLACEHOLDER",
  "FUTURE_SAFETY_GATE_CHECK_REAL_VALUE_INSERTION_DRAFT_PLACEHOLDER",
  "FUTURE_SAFETY_GATE_CHECK_OWNER_APPROVAL_PACKET_PLACEHOLDER",
  "FUTURE_SAFETY_GATE_CHECK_CONTROLLED_ONE_RUN_PREP_PLACEHOLDER"
] as const;

console.log("=== v19.6 Owner Decision Packet Safety Review Gate Validation ===\n");
...
ok(
  "doc/fixture identify v19.6 owner decision packet safety review gate correctly",
  /v19\.6 - Owner Decision Packet Safety Review Gate/.test(doc) &&
    fixtureRaw.includes("\"version\": \"v19.6\"") &&
    fixtureRaw.includes("\"executionType\": \"owner-decision-packet-safety-review-gate only\"")
);

ok(
  "v19.6 scope status flags appear",
...
if (packageParsed && typeof packageParsed === "object") {
  ...
  ok("package has test:v19.6 script", scripts["test:v19.6"] === "tsx scripts/test-v196-owner-decision-packet-safety-review-gate.mts");
}
...
if (fixtureParsed && typeof fixtureParsed === "object") {
  ...
  const boolKeys = [
    "safetyGateOnly",
    "decisionPacketSafetyReviewOnly",
    "reviewOnly",
    "placeholderOnly",
    "documentationOnly",
    "fixtureOnly",
  ...
  ok(
    "baseline confirms v13-v19.5 carry-forward",
    baseline.v13 === "CLOSED" &&
      baseline.v14 === "CLOSED" &&
      baseline.v15 === "CLOSED" &&
      ...
      baseline["v19.5DecisionPacketDraftClosure"] ===
        "OWNER FUTURE DECISION PACKET DRAFT CLOSED - READY FOR OWNER REVIEW ONLY / NO EXECUTION"
  );
  ...
  const checks = asRecord(root.futureSafetyGateCheckPlaceholders);
  ok("future safety gate check placeholders exist", safetyGateChecks.every((key) => typeof checks[key] === "object"));
  ok(
    "no safety gate check selected and placeholders only",
    safetyGateChecks.every((key) => {
      const item = asRecord(checks[key]);
      return (
        item.placeholderSafetyCheckOnly === true &&
        item.reviewGateOnly === true &&
        item.selected === false &&
  ...
  const req = asRecord(root.decisionPacketSafetyReviewRequirements);
  ok(
    "decision packet safety review requirements exist",
...
ok(
  ...
  doc.includes("V19.6 OWNER DECISION PACKET SAFETY REVIEW GATE CLOSED - READY FOR OWNER REVIEW ONLY / NO EXECUTION") &&
    fixtureRaw.includes("\"finalDecision\": \"V19.6 OWNER DECISION PACKET SAFETY REVIEW GATE CLOSED - READY FOR OWNER REVIEW ONLY / NO EXECUTION\"")
);

console.log(`\nDone v19.6 validation - ${pass} PASS, ${fail} FAIL.\n`);

/**
 * v6.4O — Synthetic Invocation Execution Candidate, Approval Required (static validation only)
...
const AUTO_INVOCATION_AUTH_PATTERNS = [
  ...
  /v6\.4O\s+approves\s+invocation\s+automatically/i,
  /candidate\s+authorizes\s+synthetic\s+invocation\s+automatically/i,
];

const HEAD_SHA = "977a21bee42bf3b708576b55e84cd4db1cbcadf8";
...
const selfSrc = readFileSync(
  "scripts/test-v64o-synthetic-invocation-execution-candidate-approval-required.mts",
  "utf8"
);
...
{
  ok("v6.4O doc exists", doc.length > 6000);
  ok("doc v6.4O label", doc.includes("v6.4O"));
...
// --- 7–15. docs safety positioning ---
{
  ok("doc HEAD 977a21b", doc.includes(HEAD_SHA) || doc.includes("977a21b"));
  ...
  ok(
    ...
    /does not invoke Gemini/i.test(doc)
  );
  ...
  ok("doc no secret use read", /does not use secrets/i.test(doc));
  ok(
...
// --- 16–18. payload contract ---
{
  ok(
    "doc future synthetic payload contract",
    /Future Synthetic Payload Contract/i.test(doc)
  );
  ok(
    "doc allowed synthetic payload fields",
    /syntheticScenarioId|syntheticIntentCategory|syntheticVehicleCategory/i.test(
      doc
    )
  );
  ok(
    "doc forbidden payload fields",
    /Raw prompt|Full user message|Full UID|Provider request\/response body/i.test(
      doc
    )
  );
}

// --- 19–34. pre-execution gates ---
{
  ok("doc pre-execution gates", /Pre-Execution Gates/i.test(doc));
  ok("doc owner approval", /PX-01.*Owner approval/i.test(doc));
  ok(
    "doc synthetic invocation approval",
    /PX-02.*Synthetic invocation approval/i.test(doc)
  );
  ok(
    "doc staging-only approval",
    /PX-03.*Staging-only environment approval/i.test(doc)
  );
  ok(
    "doc separate secret wiring approval",
    /PX-04.*Secret wiring completed separately/i.test(doc)
  );
  ok(
    "doc real provider activation approval",
    /PX-06.*Real provider activation approval/i.test(doc)
  );
  ok("doc cost cap gate", /PX-07.*Cost cap approval/i.test(doc));
  ok("doc kill switch gate", /PX-08.*Kill switch approval/i.test(doc));
  ok("doc allowlist gate", /PX-09.*Allowlist approval/i.test(doc));
  ok(
    "doc redaction logging contract approval",
    /PX-11.*Redaction\/logging contract approval/i.test(doc)
  );
  ok(
    "doc synthetic payload fixture review",
    /PX-12.*Synthetic payload fixture review/i.test(doc)
  );
  ok(
    "doc no user data guarantee",
    /PX-13.*No user data guarantee approval/i.test(doc)
  );
  ok(
    "doc monitoring metadata-only approval",
    /PX-14.*Monitoring metadata-only approval/i.test(doc)
  );
  ok(
    "doc fallback behavior approval",
    /PX-15.*Fallback behavior approval/i.test(doc)
  );
  ok("doc separate deploy approval", /PX-17.*Separate deploy approval/i.test(doc));
  ok(
    "doc separate invocation execution approval",
    /PX-18.*Separate invocation execution approval/i.test(doc)
  );
}

// --- 35–45. sequence / result / static / stop / rollback / non-auth / boundary ---
{
  ok(
    "doc future execution sequence plan only",
    /Future Execution Sequence.*Plan Only/i.test(doc) &&
      /does not execute in v6\.4O/i.test(doc)
  );
  ok(
    "doc expected future execution result contract",
    /Expected Future Execution Result Contract/i.test(doc)
  );
  ok(
    "doc realGeminiEnabled remains false in v64o",
    /realGeminiEnabled.*Must remain `false`|remains false in v6\.4O/i.test(doc)
  );
  ok(
    "doc networkCallMade remains false in v64o",
    /networkCallMade.*Must remain `false` in v6\.4O|remains false in v6\.4O/i.test(
      doc
    )
  );
  ok(
  ...
  ok("doc no user visible answer", /No user-visible answer|no user-visible answer/i.test(doc));
...
// --- package script ---
{
  ok(
    "package v64o script",
    pkg.includes(
      "test:v64o-synthetic-invocation-execution-candidate-approval-required"
    )
  );
  ok(
...
console.log(
  "\nDone v6.4O Synthetic Invocation Execution Candidate, Approval Required tests."
);
